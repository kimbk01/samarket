/**
 * Apply ONLY 20261209120000_gift_offer_room_peer_bind to linked Production DB.
 * Does NOT run `supabase db push`. Does NOT apply other pending migrations.
 *
 * Usage: node --env-file=.env.local scripts/apply-gift-offer-room-peer-bind-migration.mjs
 */
...
const { Client } = pg;
const MIGRATION_FILE = "20261209120000_gift_offer_room_peer_bind.sql";
const VERSION = "20261209120000";
...
async function verify(client) {
  ...
  if (!def.includes("not_room_peer")) {
    throw new Error("gift_certificate_offer missing not_room_peer gate");
  }
  if (!def.includes("not_room_participant")) {
    throw new Error("gift_certificate_offer missing not_room_participant gate");
  }
  if (!def.includes("v_validated_peer_user_id")) {
    throw new Error("gift_certificate_offer missing validated peer variable");
  }
  if (!def.includes("community_messenger_participants")) {
    throw new Error("gift_certificate_offer missing participant SSOT lookup");
  }
  ...
  const peerIdx = def.indexOf("not_room_peer");
  const lockIdx = def.indexOf("GIFT_LOCKED");
  if (peerIdx < 0 || lockIdx < 0 || peerIdx > lockIdx) {
    throw new Error("not_room_peer must precede GIFT_LOCKED mutation");
  }
  return {
    schema_migration_row: mig.length > 0,
    rpc_room_peer_bind: true,
    rpc_signature: "gift_certificate_offer(uuid,uuid,uuid,uuid,text)",
    live_markers: {
      not_room_peer: def.includes("not_room_peer"),
      not_room_participant: def.includes("not_room_participant"),
      validated_peer: def.includes("v_validated_peer_user_id"),
      participants: def.includes("community_messenger_participants"),
    },
  };
}
...
async function main() {
  ...
  try {
    ...
    try {
      ...
      already = proof.schema_migration_row && proof.rpc_room_peer_bind;
      if (already) {
  ...
  console.log("[apply-gift-offer-room-peer-bind-migration] ok");
}
