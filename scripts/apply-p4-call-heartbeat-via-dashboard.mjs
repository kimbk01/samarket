/**
 * Supabase Dashboard SQL Editor — P4 call heartbeat catch-up.
 * 1회차: 브라우저에서 Supabase 로그인 → 2회차부터 .playwright-supabase-profile 세션 재사용.
 *
 *   node scripts/apply-p4-call-heartbeat-via-dashboard.mjs
 */
...
const sqlPath = resolve(process.cwd(), "supabase/scripts/p4-call-heartbeat-catchup.sql");
...
async function main() {
  ...
  console.log("P4 heartbeat SQL submitted. Re-run: node scripts/verify-p4-call-heartbeat-remote.mjs");
  await context.close();
}
