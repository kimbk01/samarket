#!/usr/bin/env node
/**
 * Delivery summary snapshot RPC deploy + runtime verify.
 */
...
async function main() {
  ...
  console.log("\n=== Delivery summary snapshot RPC verify ===\n");
  ...
  const rpc0 = Date.now();
  const { data, error } = await sb.rpc("get_delivery_summary_snapshot", {
  ...
  if (error) {
    ...
    if (
      ...
      console.log("\nFAIL: get_delivery_summary_snapshot NOT deployed");
      process.exit(1);
    }
  ...
  console.log("rpc_today_sales:", data?.today_sales ?? "n/a");

  const tableProbe = await sb.from("delivery_summary_snapshots").select("store_id").limit(1);
  if (tableProbe.error?.message?.includes("does not exist")) {
    console.log("\nFAIL: delivery_summary_snapshots table missing");
    process.exit(1);
  }

  console.log("\nPASS: get_delivery_summary_snapshot deployed and callable");
}
