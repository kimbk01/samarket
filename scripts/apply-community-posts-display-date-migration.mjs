#!/usr/bin/env node
/**
 * Apply ONLY 20261230210000_community_posts_display_date.sql
 * Usage: node --env-file=.env.local scripts/apply-community-posts-display-date-migration.mjs
 */
...
const { Client } = pg;
const MIGRATION_FILE = "20261230210000_community_posts_display_date.sql";
const VERSION = "20261230210000";
  ...
  try {
    const exists = await client.query(
      `select 1 as ok from information_schema.columns
       where table_schema='public' and table_name='community_posts' and column_name='display_date'`,
    );
    if (exists.rowCount > 0) {
      console.log(JSON.stringify({ ok: true, already: true, column: "community_posts.display_date" }));
      return;
    }
    ...
    console.log(JSON.stringify({ ok: check.rowCount > 0, applied: true, version: VERSION }));
  } catch (e) {
    try {
