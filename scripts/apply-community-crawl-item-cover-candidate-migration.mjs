/**
 * Apply ONLY 20261223130000_community_crawl_item_cover_candidate.
 * Usage: node --env-file=.env.local scripts/apply-community-crawl-item-cover-candidate-migration.mjs
 */
import { readFileSync } from "node:fs";
import { resolve } from "node:path";
import { spawnSync } from "node:child_process";

const MIGRATION_FILE = "20261223130000_community_crawl_item_cover_candidate.sql";
const VERSION = "20261223130000";
...
function assertApprovedSql(sql) {
  ...
  if (!sql.includes("source_cover_candidate_url")) throw new Error("missing source_cover_candidate_url");
}
...
const check = psql(
  [
    ...
    `SELECT CASE WHEN EXISTS (
       ...
       WHERE table_schema='public' AND table_name='community_crawl_items'
         AND column_name='source_cover_candidate_url'
     ) THEN 'HAS_CANDIDATE' ELSE 'NO_CANDIDATE' END
     || '|' || CASE WHEN EXISTS (
...
const ok = check === "HAS_CANDIDATE|MIGRATION_RECORDED";
