...
async function ensureLoggedInAtStores(page) {
  ...
  if (sbUrl && sbAnon && ref) {
    ...
    const loginIds = [LOGIN_ID, "aaaa", "aaaa@manual.local"].filter(Boolean);
  ...
  const ids = [LOGIN_ID, "aaaa", "aaaa@manual.local"].filter(Boolean);
  let ok = false;
  for (const id of ids) {
...
async function runOne(page, index) {
  ...
  const philifeTab = page.locator('a.app-bottom-nav-item[href="/philife"]').first();
