#!/usr/bin/env node
/**
 * CM bootstrap critical snapshot RPC deploy + runtime verify (CMB1).
 */
...
async function main() {
  ...
  console.log("\n=== CM bootstrap snapshot RPC verify ===\n");
  ...
  const rpc0 = Date.now();
  const { data, error } = await sb.rpc("get_cm_bootstrap_critical_snapshot", {
  ...
  if (error) {
    ...
    if (
      ...
      console.log("\nFAIL: get_cm_bootstrap_critical_snapshot NOT deployed");
      process.exit(1);
    }
  ...
  console.log("rpc_rooms_len:", Array.isArray(data?.rooms) ? data.rooms.length : "n/a");
  ...
  console.log("\nPASS: get_cm_bootstrap_critical_snapshot deployed and callable");
}
