#!/usr/bin/env node
/**
 * CM bootstrap snapshot E2E verify — PASS when snapshot path active, no legacy fallback.
 */
...
function pickDevServerTerminalLog(dir) {
  ...
  const scored = fs
    ...
    .map((f) => {
      ...
      if (text.includes("[cm-bootstrap-monolith-analysis]")) score += 30;
      if (/last_command:.*verify-cm-bootstrap-snapshot/.test(meta)) score -= 200;
...
const terminalLog = process.env.CM_BOOTSTRAP_TERMINAL_LOG ?? pickDevServerTerminalLog(terminalsDir);
...
async function fetchBootstrapLite(cookie, bust = "") {
  const t0 = Date.now();
  const res = await fetch(
    `${baseUrl}/api/community-messenger/bootstrap?lite=1&fresh=1&cmBootstrapBypass=1${bust}`,
  ...
  return {
    ...
    status: res.status,
    ok: body?.ok === true,
    chatsLen: Array.isArray(body?.chats) ? body.chats.length : 0,
    groupsLen: Array.isArray(body?.groups) ? body.groups.length : 0,
    deferredCallLog: body?.deferredCallLog === true,
    hasMe: body?.me != null,
...
function assertBootstrapShape(body) {
  ...
  if (body.deferredCallLog !== true) return "deferredCallLog !== true";
  return null;
}
...
async function main() {
  ...
  console.log("\n=== CM Bootstrap Snapshot E2E Verify ===\n");
  ...
  const { error: rpcErr } = await sb.rpc("get_cm_bootstrap_critical_snapshot", {
  ...
  if (rpcErr?.message?.includes("Could not find")) {
    ...
    passes.push("get_cm_bootstrap_critical_snapshot callable");
  }
  ...
  const cold = await fetchBootstrapLite(auth.cookie, bust);
  await new Promise((r) => setTimeout(r, 350));
  const warm = await fetchBootstrapLite(auth.cookie, bust);

  const logAfter = fs.existsSync(terminalLog) ? fs.readFileSync(terminalLog, "utf8") : "";
  const newLog = logAfter.slice(logBefore.length);
  const hotpaths = parseLogBlock(newLog, "cm-bootstrap-monolith-analysis");
  const fallbacks = (newLog.match(/\[cm-bootstrap-snapshot-fallback\]/g) ?? []).length;
  ...
  else passes.push(`cold 200 (chats=${cold.chatsLen} groups=${cold.groupsLen})`);

  const shapeErr = assertBootstrapShape(cold.body);
  if (shapeErr) fails.push(`bootstrap shape: ${shapeErr}`);
  else passes.push("bootstrap shape ok");
  ...
  if (hotpaths.length === 0) {
    if (headerSnapshot && cold.rpcRemoved) {
      passes.push("[cm-bootstrap-monolith-analysis] via response headers");
    } else {
      fails.push("no [cm-bootstrap-monolith-analysis] — start npm run dev with latest code");
    }
  } else {
    passes.push("[cm-bootstrap-monolith-analysis] observed");
  }

  if (fallbacks > 0) fails.push(`legacy fallback count=${fallbacks}`);
  else passes.push("no [cm-bootstrap-snapshot-fallback]");
  ...
  console.log("\nVERDICT: PASS (CM bootstrap snapshot architecture)\n");
}
