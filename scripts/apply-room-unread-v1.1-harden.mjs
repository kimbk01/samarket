#!/usr/bin/env node
/**
 * Apply Room Unread v1.1 harden:
 *   supabase/migrations/20261014123000_dibay_append_null_sender_system_only.sql
 *
 * Requires DATABASE_URL or SUPABASE_DB_PASSWORD in .env.local
 * DO NOT use as always-on heal.
 */
...
async function main() {
  ...
  const sql = readFileSync(
    resolve(process.cwd(), "supabase/migrations/20261014123000_dibay_append_null_sender_system_only.sql"),
    "utf8"
  );
  ...
  await client.end();
  console.log("[apply-room-unread-v1.1-harden] ok — null_sender_forbidden active");
}
