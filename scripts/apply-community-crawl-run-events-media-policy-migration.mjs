/**
 * Apply ONLY 20261223120000_community_crawl_run_events_and_media_policy.
 * Does NOT run supabase db push for unrelated migrations.
 *
 * Usage: node --env-file=.env.local scripts/apply-community-crawl-run-events-media-policy-migration.mjs
 */
import { readFileSync } from "node:fs";
import { resolve } from "node:path";
import { spawnSync } from "node:child_process";

const MIGRATION_FILE = "20261223120000_community_crawl_run_events_and_media_policy.sql";
const VERSION = "20261223120000";
...
const check = psql(
  [
    ...
    `SELECT CASE WHEN EXISTS (
       ...
       WHERE table_schema='public' AND table_name='community_crawl_run_events'
     ) THEN 'HAS_EVENTS' ELSE 'NO_EVENTS' END
     || '|' || CASE WHEN EXISTS (
       SELECT 1 FROM information_schema.columns
       WHERE table_schema='public' AND table_name='community_crawl_sources'
         AND column_name='media_policy'
     ) THEN 'HAS_MEDIA_POLICY' ELSE 'NO_MEDIA_POLICY' END
     || '|' || CASE WHEN EXISTS (
...
const ok = check === "HAS_EVENTS|HAS_MEDIA_POLICY|MIGRATION_RECORDED";
