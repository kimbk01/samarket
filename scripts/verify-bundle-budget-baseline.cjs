 ...
 * Structural guard: provenance.bytes round-trip + messenger ⊆ total (prevents mixed-build baselines).
 */
const fs = require("fs");
...
async function main() {
  ...
  const { validateBaselineIntegrity } = await import("./lib/bundle-budget-metrics.mjs");
  const integrity = validateBaselineIntegrity(data);
  if (!integrity.ok) {
