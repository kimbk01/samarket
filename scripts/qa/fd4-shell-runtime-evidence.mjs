#!/usr/bin/env node
/**
 * FD4 App Shell runtime evidence.
 * DeviceClass from native plugin. Shell family from inlined FD4 contract.
 * Does not reopen FD1/FD2/FD3 or domain dual-pane.
 */
...
const OUT = path.join(ROOT, ".tmp/fd4-shell-runtime");
...
const DEVICES = {
  samsung: { id: "samsung", serial: "RFCY40PY2CA", label: "Samsung SM-M156S", expectedClass: "PHONE_ANDROID", expectedFamily: "PHONE", cdpPort: 9364 },
  xiaomi: { id: "xiaomi", serial: "8b37179f7d94", label: "Xiaomi 24076RP19G", expectedClass: "TABLET_ANDROID", expectedFamily: "TABLET", cdpPort: 9365 },
};

const ROUTES = ["/", "/market", "/community-messenger", "/stores", "/mypage"];
...
const READ_EXPR = `(() => {
  const run = async () => {
    ...
    const bottomNav = document.querySelector(".app-bottom-nav-shell");
    const desktopRail = document.querySelector(".app-desktop-side-nav, [data-main-desktop-side-nav]");
    const tabletNav = document.querySelector("[data-tablet-nav], .tablet-nav");
    const familyFromClass = (deviceClass) => {
    ...
    const out = {
      ...
      appOrientation: Number(window.innerWidth || 0) > Number(window.innerHeight || 0) ? "landscape" : "portrait",
      attrDeviceClass: shellEl ? shellEl.getAttribute("data-dibay-device-class") : null,
      attrShellFamily: shellEl ? shellEl.getAttribute("data-dibay-shell-family") : null,
      attrNavPresentation: shellEl ? shellEl.getAttribute("data-dibay-nav-presentation") : null,
      attrLayoutMode: shellEl ? shellEl.getAttribute("data-dibay-layout-mode") : null,
      hasAppShell: Boolean(shellEl),
      hasBottomNav: Boolean(bottomNav),
      hasDesktopRail: Boolean(desktopRail),
      hasTabletNav: Boolean(tabletNav),
    };
    try {
    ...
    out.expectedFamily = familyFromClass(out.deviceClass);
    out.headJsWired = Boolean(out.attrDeviceClass);
    return out;
  };
  return run();
})()`;
...
async function waitForAppPage(device, attempts = 8) {
  let last = null;
  for (let i = 0; i < attempts; i += 1) {
    last = await readAndroid(device);
    if (
      last?.href &&
      !/about:blank/i.test(last.href) &&
      last.deviceClass &&
      (last.hasAppShell || last.hasBottomNav)
    ) {
      return last;
    }
    sleep(800);
...
async function gotoAndRead(device, pathname) {
  const cdp = await connectAndroidCdp(device.serial, device.cdpPort);
  try {
    await cdp.eval(`(() => { location.assign(${JSON.stringify(pathname)}); return true; })()`);
  } finally {
    cdp.close();
  }
  sleep(2200);
  return readAndroid(device);
}

function judgeSample(sample, device) {
  const classOk = sample?.deviceClass === device.expectedClass;
  const familyOk = sample?.expectedFamily === device.expectedFamily;
  const noWrongNav = sample?.hasTabletNav !== true && sample?.hasDesktopRail !== true;
  const navOk = sample?.hasBottomNav === true;
  const attrOk =
    !sample?.headJsWired ||
    (sample.attrDeviceClass === device.expectedClass && sample.attrShellFamily === device.expectedFamily);
  return {
    deviceClass: classOk,
    shellFamily: familyOk,
    existingBottomNav: navOk,
    noInventedRail: noWrongNav,
    headAttrMatch: attrOk,
    headJsWired: Boolean(sample?.headJsWired),
    result: classOk && familyOk && navOk && noWrongNav && attrOk ? "PASS" : "FAIL",
  };
}
...
async function observeSamsung(device) {
  ...
  sleep(2500);
  const firstPaint = await waitForAppPage(device, 3);
  const settledEarly = await waitForAppPage(device, 10);
  const routes = {};
  for (const route of ROUTES) {
    routes[route] = await gotoAndRead(device, route);
  }
  restoreRotation(device.serial);
  const settled = settledEarly?.hasAppShell ? settledEarly : routes["/"];
  const judged = judgeSample(settled, device);
  const firstPaintOk =
    firstPaint?.hasTabletNav !== true &&
    firstPaint?.hasDesktopRail !== true &&
    firstPaint?.expectedFamily !== "TABLET" &&
    firstPaint?.expectedFamily !== "DESKTOP" &&
    firstPaint?.attrShellFamily !== "TABLET" &&
    firstPaint?.attrShellFamily !== "DESKTOP";
  const routeOk = ROUTES.every((route) => judgeSample(routes[route], device).result === "PASS");
  const record = {
    DEVICE: device.label,
    SERIAL: device.serial,
    EXPECTED: { deviceClass: device.expectedClass, shellFamily: device.expectedFamily },
    FIRST_PAINT: firstPaint,
    SETTLED: settled,
    ROUTES: routes,
    JUDGE: {
      ...judged,
      firstPaintNoWrongNav: firstPaintOk,
      routeSmoke: routeOk,
      result: judged.result === "PASS" && firstPaintOk && routeOk ? "PASS" : "FAIL",
    },
    RESULT: judged.result === "PASS" && firstPaintOk && routeOk ? "PASS" : "FAIL",
  };
...
async function observeXiaomi(device) {
  ...
  launchApp(device.serial);
  sleep(2500);
  const firstPaint = await readAndroid(device);
  sleep(2000);
  ...
  restoreRotation(device.serial);
  const judgedPortrait = judgeSample(portrait, device);
  const judgedLandscape = judgeSample(landscape, device);
  const judgedReturn = judgeSample(returnPortrait, device);
  const firstPaintOk =
    firstPaint?.hasTabletNav !== true &&
    firstPaint?.hasDesktopRail !== true &&
    firstPaint?.expectedFamily !== "PHONE" &&
    firstPaint?.attrShellFamily !== "PHONE";
  const noDemote =
    portrait?.expectedFamily === "TABLET" &&
    landscape?.expectedFamily === "TABLET" &&
    returnPortrait?.expectedFamily === "TABLET";
  const record = {
    DEVICE: device.label,
    SERIAL: device.serial,
    EXPECTED: { deviceClass: device.expectedClass, shellFamily: device.expectedFamily },
    ...
    PORTRAIT: portrait,
    LANDSCAPE: landscape,
    RETURN_PORTRAIT: returnPortrait,
    JUDGE: {
      portrait: judgedPortrait,
      landscape: judgedLandscape,
      returnPortrait: judgedReturn,
      firstPaintNoPhoneNav: firstPaintOk,
      familyUnchangedOnRotate: noDemote,
      result:
        judgedPortrait.result === "PASS" &&
        judgedLandscape.result === "PASS" &&
        judgedReturn.result === "PASS" &&
        noDemote
          ? "PASS"
          : "FAIL",
    },
...
async function evalIosPage(wsUrl) {
  ...
  try {
    ...
    await call("Runtime.evaluate", {
      expression: `(() => {
        window.__fd4shell = { started: true };
        Promise.resolve(${READ_EXPR}).then((r) => {
          window.__fd4shell = { done: true, sample: r };
        }).catch((e) => {
          window.__fd4shell = { done: true, nativeError: String(e && e.message ? e.message : e) };
        });
        return true;
      })()`,
      returnByValue: true,
    ...
    for (let i = 0; i < 40; i += 1) {
      const poll = await call("Runtime.evaluate", {
        expression: "JSON.stringify(window.__fd4shell||null)",
        returnByValue: true,
      });
...
async function observeIos() {
  ...
  const judged = judgeSample(sample, { expectedClass: "PHONE_IOS", expectedFamily: "PHONE" });
  const record = {
    DEVICE: "iPhonebk",
    UDID: IOS_UDID,
    CURRENT: sample,
    ...
    NOTE: "Physical rotate remains FD3 NOT_PROVEN. FD4 does not promote it.",
  };
...
async function main() {
  ...
  const out = {
    ...
    fd2Reopened: false,
    fd3Reopened: false,
    productionWeb: "samarket.vercel.app",
  };
