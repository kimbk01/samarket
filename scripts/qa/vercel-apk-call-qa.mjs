#!/usr/bin/env node
/**
 * Vercel-origin APK 2-device call QA.
 * 기본: 기기 WebView 에 이미 로그인된 실제 계정 사용 (강제 aaaa/qqqq 주입 없음).
 * Usage: node scripts/qa/vercel-apk-call-qa.mjs
 * Optional: P4_FORCE_AAAA_QQQQ=1 — legacy aaaa/qqqq 강제 로그인
 */
...
/** 기본: 기기 WebView 에 로그인된 실제 계정 사용. aaaa/qqqq 강제 주입은 P4_FORCE_AAAA_QQQQ=1 */
const FORCE_AAAA_QQQQ = process.env.P4_FORCE_AAAA_QQQQ === "1";
const LOGIN_A = process.env.P4_CALLER_LOGIN?.trim() || process.env.P4_LOGIN_A?.trim() || "aaaa";
const LOGIN_B = process.env.P4_CALLEE_LOGIN?.trim() || process.env.P4_LOGIN_B?.trim() || "qqqq";
const USER_A_OVERRIDE = process.env.P4_CALLER_USER_ID?.trim() || process.env.P4_INITIATOR_USER_ID?.trim() || "";
const USER_B_OVERRIDE = process.env.P4_CALLEE_USER_ID?.trim() || process.env.P4_PEER_USER_ID?.trim() || "";
...
async function ensureDeviceLogins() {
  ...
  if (FORCE_AAAA_QQQQ) {
    log(`FORCE aaaa/qqqq login A=${SERIAL_A} login=${LOGIN_A} B=${SERIAL_B} login=${LOGIN_B}`);
    const expectedA = USER_A_OVERRIDE || (await resolveUserId(LOGIN_A));
    const expectedB = USER_B_OVERRIDE || (await resolveUserId(LOGIN_B));
    ...
    USER_A = loginA.probe.userId;
    USER_B = loginB.probe.userId;
    ACCOUNT_A = LOGIN_A;
    ACCOUNT_B = LOGIN_B;
  ...
  log(`use device session SSOT A=${SERIAL_A} B=${SERIAL_B} (no forced aaaa/qqqq inject)`);
  ...
  if (!probeA.ok || !probeA.userId) {
    throw new Error(
      `device A not logged in — log in manually on APK or set P4_FORCE_AAAA_QQQQ=1 / P4_CALLER_LOGIN`,
    );
  }
  if (!probeB.ok || !probeB.userId) {
    throw new Error(
      `device B not logged in — log in manually on APK or set P4_FORCE_AAAA_QQQQ=1 / P4_CALLEE_LOGIN`,
    );
  }
  USER_A = USER_A_OVERRIDE || probeA.userId;
  USER_B = USER_B_OVERRIDE || probeB.userId;
  ACCOUNT_A = probeA.username || LOGIN_A;
  ACCOUNT_B = probeB.username || LOGIN_B;
  if (USER_A === USER_B) {
    throw new Error(`caller/callee same userId=${USER_A} — use two different accounts on A/B`);
  }
  log(`device sessions A=${ACCOUNT_A}(${USER_A}) B=${ACCOUNT_B}(${USER_B})`);

  const regA = await refreshApkFcmRegisterTail({
    adb: adbFn,
    serial: SERIAL_A,
    pkg: PKG,
    act: ACT,
  ...
  const regB = await refreshApkFcmRegisterTail({
    adb: adbFn,
    serial: SERIAL_B,
    pkg: PKG,
    act: ACT,
  ...
  const fcmA = regA.fcmUserId ?? lastPushRegisterUserId(regA.registerLogcat);
  const fcmB = regB.fcmUserId ?? lastPushRegisterUserId(regB.registerLogcat);
  log(`FCM tail A=${fcmA ?? "?"} B=${fcmB ?? "?"}`);
}
...
async function main() {
  ...
  log(`QA accounts caller=${ACCOUNT_A}(${initiatorId}) callee=${ACCOUNT_B}(${recipientId})`);
  ...
  const report = {
    vercel: VERCEL,
    accounts: {
      caller: { login: ACCOUNT_A, userId: initiatorId },
      callee: { login: ACCOUNT_B, userId: recipientId },
      sessionSource: FORCE_AAAA_QQQQ ? "forced_login" : "device_webview_probe",
    },
