#!/usr/bin/env node
/**
 * ARO-OPS-UX-001-W1 — Production light proof (non-destructive).
 * Checks posts-management adopts shared contract markers + tablet geometry.
 */
...
const OUT = resolve(process.cwd(), "docs/perf/admin-aro-ops-ux-001-w1");
const EMAIL = process.env.E2E_ADMIN_EMAIL || process.env.QA_ADMIN_EMAIL || "aaaa@manual.local";
const EXPECT_SHA = (process.env.ARO_OPS_UX_W1_EXPECT_SHA || "").slice(0, 9);
...
async function main() {
  ...
  if (!auth?.session) {
    writeFileSync(
      resolve(OUT, "aro-ops-ux-001-w1-prod-light.json"),
  ...
  await page.waitForSelector("[data-admin], [data-aro-ops-ux-001-w1='1']", { timeout: 90000 });
  ...
  const geo = await page.evaluate(() => {
    ...
    return {
      ...
      hasRoot: !!document.querySelector("[data-aro-ops-ux-001-w1='1']"),
      hasViewport: !!vp,
  ...
  // Select all current page if rows exist
  ...
  await page.screenshot({ path: resolve(OUT, "prod-posts-management-w1.png"), fullPage: true });

  const checks = {
    page: page.url().includes("/admin/posts-management"),
    w1Root: geo.hasRoot,
  ...
  writeFileSync(resolve(OUT, "aro-ops-ux-001-w1-prod-light.json"), JSON.stringify(report, null, 2));
