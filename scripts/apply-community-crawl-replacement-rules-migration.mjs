/**
 * Apply ONLY 20261227120000_community_crawl_replacement_rules.
 * Usage: node --env-file=.env.local scripts/apply-community-crawl-replacement-rules-migration.mjs
 */
import { readFileSync } from "node:fs";
import { resolve } from "node:path";
import { spawnSync } from "node:child_process";

const MIGRATION_FILE = "20261227120000_community_crawl_replacement_rules.sql";
const VERSION = "20261227120000";
...
function assertApprovedSql(sql) {
  ...
  if (!sql.includes("community_crawl_replacement_rules")) {
    throw new Error("missing replacement rules table");
  }
  if (sql.includes("community_post_images")) {
    throw new Error("must not touch community_post_images");
  }
}
...
const check = psql(
  [
    ...
    "-At",
    "-c",
    `SELECT to_regclass('public.community_crawl_replacement_rules') IS NOT NULL`,
  ],
  env
);
console.log(
  JSON.stringify({
    version: VERSION,
    tablePresent: check === "t",
    ok: check === "t",
  })
);
if (check !== "t") process.exit(1);
