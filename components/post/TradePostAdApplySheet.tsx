    ...
    >
      <div className="w-full max-w-xl rounded-t-[length:var(--ui-radius-rect)] bg-sam-surface px-4 pb-6 pt-4 shadow-2xl">
        <div className="mb-2 flex items-center justify-between">
          <h2 className="text-[16px] font-semibold text-sam-fg">유료 광고 신청</h2>
          <button
            type="button"
            onClick={onClose}
            className="rounded border border-sam-border px-2 py-1 text-[12px] text-sam-muted"
          >
            닫기
          </button>
        </div>
        ...
        {loading ? (
          <p className="py-8 text-center text-[13px] text-sam-muted">불러오는 중…</p>
        ) : (
          <>
            {err ? (
              <p className="mb-3 rounded-ui-rect bg-red-50 px-3 py-2 text-[12px] text-red-700">{err}</p>
            ) : null}
            {message ? (
              <p className="mb-3 rounded-ui-rect bg-emerald-50 px-3 py-2 text-[12px] text-emerald-700">{message}</p>
            ) : null}
            {data?.criteriaSummary ? (
              <p className="mb-3 rounded-ui-rect bg-sam-app px-3 py-2 text-[12px] text-sam-muted">{data.criteriaSummary}</p>
            ) : null}
            {Array.isArray(data?.formatGuide) && data?.formatGuide.length > 0 ? (
              <ul className="mb-3 list-disc space-y-1 pl-5 text-[12px] text-sam-muted">
            ...
            {products.length === 0 ? (
              <p className="py-8 text-center text-[13px] text-sam-muted">신청 가능한 광고 상품이 없습니다.</p>
            ) : (
                    ...
                    <div key={p.id} className="rounded-ui-rect border border-sam-border px-3 py-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          ...
                          {p.description ? (
                            <p className="mt-0.5 text-[12px] text-sam-meta">{p.description}</p>
                          ) : null}
                        </div>
                        <div className="shrink-0 text-right">
                          <p className="text-[15px] font-bold text-sam-fg">{p.point_cost.toLocaleString()}P</p>
                          <button
                            type="button"
                            disabled={disabled}
                            onClick={() => void apply(p.id)}
                            className="mt-2 rounded-ui-rect bg-sam-ink px-3 py-1.5 text-[12px] font-semibold text-white disabled:opacity-40"
                          >
                      ...
                      {!p.eligible && p.reason ? (
                        <p className="mt-2 rounded-ui-rect bg-amber-50 px-2 py-1 text-[11px] text-amber-800">
                          신청 불가: {p.reason}
                        </p>
                      ) : null}
                      {Array.isArray(p.checks) && p.checks.length > 0 ? (
                        <div className="mt-2 rounded-ui-rect bg-sam-app px-2 py-2 text-[11px]">
