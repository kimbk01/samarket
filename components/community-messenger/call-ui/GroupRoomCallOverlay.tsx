...
export function GroupRoomCallOverlay({
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-end justify-center bg-black/40 px-4 pb-4 sm:items-center sm:pb-0">
      <div className="w-full max-w-[420px] rounded-ui-rect border border-ui-border bg-ui-surface px-5 pb-5 pt-6 text-ui-fg shadow-[var(--ui-shadow-card)]">
        <div className="flex items-center justify-between gap-3">
          <span className="rounded-ui-rect border border-ui-border bg-ui-page px-3 py-1 text-[12px] font-semibold text-ui-fg">
            {isGroupRoom ? groupPrefix : ""}
            {sessionPanel.kind === "video" ? t("nav_video_call_label") : t("nav_voice_call_label")}
          </span>
          {sessionPanel.mode === "active" ? (
            <span className="rounded-ui-rect border border-ui-border bg-ui-surface px-3 py-1 text-[12px] font-semibold text-ui-fg">
          ...
          {sessionPanel.kind === "video" ? (
            <div className="relative min-h-[250px] bg-black">
              {groupCall.localStream ? (
                ...
                <div className="flex h-[250px] flex-col items-center justify-center gap-3 bg-neutral-950 px-6 text-center">
                  <div className="flex h-16 w-16 items-center justify-center rounded-ui-rect border border-white/12 bg-white/5 text-[12px] font-semibold text-white">
                    VIDEO
                  </div>
            ...
            <div className="flex h-[250px] flex-col items-center justify-center gap-4 bg-neutral-950">
        ...
        <div className="mt-5 text-center">
          <h2 className="text-[22px] font-semibold text-ui-fg sm:text-[24px]">{sessionPanel.peerLabel}</h2>
          <p className="mt-1 text-[14px] text-ui-muted">{groupCall.callStatusLabel}</p>
          {groupCall.connectionBadge ? (
            <p className="mt-3 inline-flex rounded-ui-rect border border-ui-border bg-ui-page px-3 py-1 text-[12px] font-semibold text-ui-fg">
            ...
            <div className="mt-3 flex flex-wrap justify-center gap-2">
              {groupCall.participants.map((participant) => (
                <span
                  ...
                  className="rounded-ui-rect border border-ui-border bg-ui-page px-3 py-1 text-[11px] font-semibold text-ui-fg"
                >
        ...
        {groupCall.errorMessage ? (
          <div className="mt-4 rounded-ui-rect border border-ui-border bg-ui-page p-4 text-left">
            <p className="text-[13px] font-semibold text-ui-fg">{groupCall.errorMessage}</p>
            {permissionGuide?.description ? <p className="mt-2 text-[12px] text-ui-muted">{permissionGuide.description}</p> : null}
            <div className="mt-3 flex gap-2">
              <button
                ...
                className="flex-1 rounded-ui-rect border border-ui-border bg-ui-surface px-4 py-3 text-[13px] font-semibold text-ui-fg disabled:opacity-40"
              >
              ...
              <button
                ...
                className="rounded-ui-rect border border-ui-border bg-ui-surface px-4 py-3 text-[13px] font-medium text-ui-muted"
              >
        ...
        <div className="mt-5">
          <div className="mb-2 flex items-center justify-between gap-3">
            <p className="text-[11px] font-semibold tracking-[0.02em] text-ui-muted">통화 제어</p>
          ...
          <div className="rounded-ui-rect border border-ui-border bg-ui-page p-3">
            {sessionPanel.mode === "incoming" ? (
