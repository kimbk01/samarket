...
export function PostSellerTradeStrip({
  ...
  return (
    <div className={`border-b border-violet-100 bg-violet-50/60 ${pad}`}>
      <p className={`${titleCls} font-medium text-violet-900`}>구매자 채팅 · 거래</p>
      ...
      <ul className="mt-2 space-y-1.5">
        {rows.map((r) => {
          ...
          return (
            <li key={r.chatId}>
              <div className="flex flex-col gap-1.5 rounded-lg border border-violet-200 bg-white px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
                <Link
                  href={`/chats/${r.chatId}`}
                  className="min-w-0 flex-1 text-[13px] text-gray-900 active:bg-violet-50"
                >
                  <span className="truncate font-medium">{r.buyerNickname}</span>
                  <span className="mt-0.5 block text-[11px] text-violet-700 sm:mt-0 sm:inline sm:ml-2">
                ...
                {canSellerComplete ? (
                  <button
                    ...
                    className="shrink-0 rounded-lg bg-violet-600 px-3 py-1.5 text-[12px] font-medium text-white disabled:opacity-50"
                  >
