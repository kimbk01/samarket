...
import { resolveMessagingGlobalChromePolicy } from "@/lib/layout/messaging-global-chrome-policy";
 ...
 * 마운트 기준은 `resolveMessagingGlobalChromePolicy` 단일 정책.
 */
export function MessagingGlobalChrome({ regionBarInLayout }: { regionBarInLayout: boolean }) {
  const pathname = usePathname();
  const p = useMemo(
    () => resolveMessagingGlobalChromePolicy(pathname, regionBarInLayout),
