...
export function IncomingCallFullScreen({
  peerLabel,
  peerPublicId,
  peerAvatarUrl,
  callKind,
  ...
  return (
    <div className="relative flex h-full min-h-[100dvh] flex-col overflow-hidden bg-[#121212] px-[clamp(1rem,4vw,1.5rem)] pb-[max(28px,calc(var(--safe-bottom)+16px))] pt-[max(32px,calc(var(--safe-top)+20px))] text-white">
      {peerAvatarUrl ? (
        ...
        <div
          className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_22%,rgba(0,117,74,0.28),transparent_42%),linear-gradient(180deg,#121212_0%,#1E1E1E_58%,#121212_100%)]"
          aria-hidden
        />
      )}
      <div className="relative z-[1] flex min-h-0 flex-1 flex-col items-center justify-center text-center">
        <div className="relative flex h-32 w-32 items-center justify-center">
          ...
          <div className="relative z-[2] h-24 w-24 overflow-hidden rounded-full bg-[#F1F8F4] shadow-[0_18px_46px_rgba(0,0,0,0.35)] ring-2 ring-white/16">
            <SamarketThumbnail
              ...
              fallbackSrc=""
              fallbackNode={
                <span className="flex h-full w-full items-center justify-center text-[clamp(1.5rem,6vw,2rem)] font-bold text-[#00754A]">
        ...
        {normalizedPublicId ? (
          <p className="mt-1 max-w-full truncate text-[clamp(0.875rem,3.4vw,1rem)] font-medium text-white/68">
            @{normalizedPublicId}
          </p>
        ) : null}
        <p className="mt-4 text-[clamp(0.9375rem,3.6vw,1.125rem)] font-semibold text-white/88">{kindLabel}</p>
      </div>
      <div className="relative z-[1] mt-auto w-full">
        <IncomingCallControls
          acceptLabel={safeT("cm_ui_incoming_accept_call_label", {
            fallbackKo: "통화",
            fallbackEn: "Call",
          })}
