...
/**
 * Incoming call UI. When `vm.incomingDesktopChrome`, renders desktop-style window
 * (min / max / close + four bottom actions). Otherwise legacy mobile layout.
 */
export function IncomingCallView({ vm }: { vm: CallScreenViewModel }) {
  ...
  const chrome = vm.incomingDesktopChrome === true;
      ...
      <div className="relative z-[2] flex min-h-0 flex-1 flex-col px-5 pb-[max(18px,calc(env(safe-area-inset-bottom)+10px))] pt-2">
        ...
        <div className="flex flex-1 flex-col items-center justify-center">
          ...
          <div className="mt-8 text-center">
            <p className="text-[18px] font-semibold text-white">{peerName}</p>
            <p className="mt-2 text-[14px] text-white/70">{statusLine}</p>
            {vm.subStatusText ? (
              <p className="mt-2 text-[12px] text-amber-200/90">{vm.subStatusText}</p>
            ) : null}
          </div>
        </div>
          ...
          <div className="flex flex-1 flex-col items-center gap-2">
            <div className="relative">
              <button
                ...
                className={`${actionCircleBase} bg-zinc-700 text-white ${vm.mode !== "video" ? "opacity-50" : ""}`}
    ...
    <div className="relative z-[2] flex min-h-0 flex-1 flex-col bg-black/55 px-3 pb-[max(16px,env(safe-area-inset-bottom))] pt-2 backdrop-blur-[2px]">
      ...
      {vm.secondaryActions?.length ? (
        <div className="mt-2 flex justify-center gap-3">
          {vm.secondaryActions.map((a) => (
            <button
              key={a.id}
              type="button"
              disabled={a.disabled}
              onClick={() => a.onClick()}
              className="text-[12px] font-medium text-white/70 underline-offset-2 hover:text-white"
            >
              {a.label}
            </button>
          ))}
