    ...
    <section className={`border-b border-sam-border-soft bg-sam-surface px-4 py-4 ${disabled ? "opacity-60" : ""}`}>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {value.map((item, index) => (
          <div
            key={index}
            className="relative h-24 w-24 shrink-0 overflow-hidden rounded-ui-rect bg-sam-surface-muted"
          >
        ...
        {value.length < maxCount && !disabled && (
          <button
            ...
            className="flex h-24 w-24 shrink-0 items-center justify-center rounded-ui-rect border-2 border-dashed border-sam-border text-sam-meta hover:border-sam-border"
          >
            +
          </button>
        )}
