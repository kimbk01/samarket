  ...
  const load = useCallback(async (opts?: { force?: boolean }) => {
    ...
    try {
      ...
      if (handoff) {
        ...
        let national: { status: "resolved" | "ambiguous" | "unresolved"; tradeLguId: string | null } = {
          status: "unresolved",
          tradeLguId: null,
        };
        try {
          ...
          const row =
            listResult.ok && handoff.addressId
              ? listResult.rows.find((a) => a.id === handoff.addressId) ?? null
              : null;
          if (row) national = await resolveNationalLguFromAddress(row);
        } catch {
          /* national resolve best-effort */
        }
        if (requestGeneration !== requestGenerationRef.current) return;
        const nationalOk = national.status === "resolved" && !!national.tradeLguId;
        resolvedRef.current?.({
          ready: true,
          missing: !line || !nationalOk,
          displayLine: line || null,
          regionId: handoff.regionId,
          cityId: handoff.cityId,
