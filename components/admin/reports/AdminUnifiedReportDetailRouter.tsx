...
export function AdminUnifiedReportDetailRouter({ reportId }: { reportId: string }) {
  ...
  if (phase === "loading") {
    return <div className="py-8 text-center text-[14px] text-gray-500">불러오는 중…</div>;
  }
  if (phase === "none") {
    return <div className="py-8 text-center text-[14px] text-gray-500">신고를 찾을 수 없습니다.</div>;
  }
  if (phase === "legacy") {
