...
export function AdminPointLedgerPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="sam-text-page-title font-semibold text-sam-fg"> {t("admin_points_ledger_page")}
      </h1>
      {entries.length === 0 ? (
        <div className="rounded-ui-rect border border-sam-border bg-sam-surface py-12 text-center sam-text-body text-sam-muted"> {t("admin_points_ledger_empty")}
        </div>
      ) : (
          ...
          <table className="w-full min-w-[640px] border-collapse sam-text-body">
            <thead>
              <tr className="border-b border-sam-border bg-sam-app">
                ...
                <th className="px-3 py-2.5 text-left font-medium text-sam-fg"> {t("admin_points_th_amount")}
                </th>
                <th className="px-3 py-2.5 text-left font-medium text-sam-fg"> {t("admin_points_th_balance")}
                </th>
                <th className="px-3 py-2.5 text-left font-medium text-sam-fg"> {t("admin_points_th_description")}
                </th>
              ...
              {entries.map((e) => (
                ...
                >
                  ...
                  <td className="px-3 py-2.5 text-sam-fg">
                    {pointLedgerTypeLabel(t, e.entryType)}
                  </td>
                  <td
