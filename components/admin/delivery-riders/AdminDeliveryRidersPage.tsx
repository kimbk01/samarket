...
export function AdminDeliveryRidersPage() {
    ...
    void fetch("/api/admin/delivery-riders", { cache: "no-store" })
      ...
      .then(async (r) => {
        ...
        if (!r.ok) {
          ...
          setError(r.status === 503 ? "RPC 미배포(마이그레이션)" : `불러오기 실패 (${r.status})`);
    ...
    async (orderId: string, body: Record<string, unknown>) => {
      ...
      try {
        ...
        if (!r.ok) {
          ...
          setError(j?.error ? String(j.error) : `배달 PATCH ${r.status}`);
    ...
    async (riderId: string, body: Record<string, unknown>) => {
      ...
      try {
        ...
        if (!r.ok) {
          ...
          setError(j?.error ? String(j.error) : `라이더 PATCH ${r.status}`);
  ...
  const markAttention = useCallback(async (orderId: string) => {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        setError(j?.error ? String(j.error) : `주문 PATCH ${r.status}`);
  ...
  return (
    ...
    <div className="sam-page-stack">
      ...
      <AdminPageHeader
        ...
        description="배차·상태·실패 처리 · 단일 스냅샷 RPC · 주문 상태 머신은 변경하지 않음"
      ...
      <div className="flex flex-wrap items-center gap-2 sam-text-xxs text-sam-muted">
        ...
        <span>
          ...
          운영 안정성(휴리스틱) <strong className="text-sam-fg">{stabPct}%</strong>

        </span>

        {generatedAt ? <span className="font-mono text-[10px]">생성 {fmtTs(generatedAt)}</span> : null}
          ...
          <table className="min-w-[1100px] w-full border-collapse sam-text-xxs">
            ...
            <thead>
              ...
              <tr className="border-b border-sam-border text-left text-sam-muted">
                ...
                <th className="py-2 px-2">라이더</th>

                <th className="py-2 px-2">상태</th>

                <th className="py-2 px-2">진행</th>

                <th className="py-2 px-2">오늘완료</th>

                <th className="py-2 px-2">평균(분)</th>

                <th className="py-2 px-2">실패행</th>

                <th className="py-2 px-2">장기</th>

                <th className="py-2 px-2">활동·위치</th>
              ...
              {riders.map((r) => (
                ...
                <tr key={r.id} className="border-b border-sam-border/60 align-top">
                  ...
                  <td className="py-2 px-2">
                    ...
                    <div>{r.is_online ? "온라인" : "오프라인"}</div>

                    <div className="text-[10px] text-sam-muted">adm:{r.admin_status ?? "—"}</div>

                    {r.suspended_at ? <div className="text-[10px] text-sam-warning">정지</div> : null}
                  ...
                  <td className="py-2 px-2 tabular-nums">
                    ...
                    {r.avg_delivery_minutes_today == null ? "—" : String(r.avg_delivery_minutes_today)}
                  ...
                  <td className="py-2 px-2">
                    ...
                    <details className="mt-1">
                      ...
                      <summary className="cursor-pointer text-[10px] text-signature">메모</summary>
          ...
          <table className="min-w-[920px] w-full border-collapse sam-text-xxs">
            ...
            <thead>
              ...
              <tr className="border-b border-sam-border text-left text-sam-muted">
                ...
                <th className="py-2 px-2">주문</th>

                <th className="py-2 px-2">주문상태</th>

                <th className="py-2 px-2">배달상태</th>

                <th className="py-2 px-2">갱신</th>

                <th className="py-2 px-2">배정</th>
            ...
            <tbody>
              ...
              {unassigned.map((u) => (
                ...
                <tr key={u.order_id} className="border-b border-sam-border/60 align-top">
                  ...
                  <td className="py-2 px-2">{u.order_status ?? "—"}</td>

                  <td className="py-2 px-2">{u.delivery_status ?? "—"}</td>
                    ...
                    >
                      ...
                      {assignableRiders.map((rr) => (
                        ...
                        <option key={rr.id} value={rr.id}>
                          ...
                          {(rr.display_name ?? rr.id.slice(0, 8)) + (rr.is_online ? "" : " · 오프라인")}
        ...
        <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/40 p-3 sm:items-center">
          ...
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-ui-rect border border-sam-border bg-sam-surface p-4 shadow-lg">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <h2 className="sam-text-body font-medium text-sam-fg">배달 상세</h2>
            ...
            {detailLoading ? (
              ...
              <p className="mt-4 text-sam-muted sam-text-xxs">불러오는 중…</p>
              ...
              <dl className="mt-3 space-y-2 sam-text-xxs">
                ...
                <div className="flex justify-between gap-2">
                  ...
                  <dd className="font-mono text-sam-fg">{detail.rider_id ?? "—"}</dd>
                ...
                <div>
                  ...
                  <dt className="text-sam-muted">POD 수령자</dt>

                  <dd className="mt-1 text-sam-fg">{detail.delivered_receiver_name ?? "—"}</dd>

                </div>

                <div>

                  <dt className="text-sam-muted">POD 메모</dt>

                  <dd className="mt-1 whitespace-pre-wrap">{detail.delivered_proof_note ?? "—"}</dd>

                </div>

                <div>

                  <dt className="text-sam-muted">POD 위치</dt>
                ...
                {detail.delivered_proof_admin_view_url ? (
                  ...
                  <div>
                    ...
                    <dt className="text-sam-muted">POD 사진</dt>
                    ...
                    <dd className="mt-2 space-y-1">
                      ...
                      {detail.delivered_proof_admin_view_legacy_public ? (
                        ...
                        <span className="sam-text-xxs text-sam-muted">서명 URL (약 10분 후 만료 — 새로고침으로 재발급)</span>
                ...
                <div>
                  ...
                  <dt className="text-sam-muted">라이더 실패 보고 시각</dt>
                ...
                <div>
                  ...
                  <dt className="text-sam-muted">라이더 실패 사유</dt>

                  <dd className="mt-1 whitespace-pre-wrap">{detail.rider_failure_report_reason ?? "—"}</dd>

                </div>

                <div>

                  <dt className="text-sam-muted">실패 증빙 메모</dt>

                  <dd className="mt-1 whitespace-pre-wrap">{detail.failure_note ?? "—"}</dd>

                </div>

                <div>

                  <dt className="text-sam-muted">실패 증빙 위치</dt>
                ...
                {detail.failure_proof_admin_view_url ? (
                  ...
                  <div>
                    ...
                    <dt className="text-sam-muted">실패 증빙 사진</dt>
                    ...
                    <dd className="mt-2 space-y-1">
                      ...
                      {detail.failure_proof_admin_view_legacy_public ? (
                        ...
                        <span className="sam-text-xxs text-sam-muted">서명 URL (약 10분 후 만료)</span>
                ...
                <div className="flex justify-between gap-2">
                  ...
                  <dt className="text-sam-muted">failed_at (확정)</dt>
                ...
                <div>
                  ...
                  <dt className="text-sam-muted">실패 사유</dt>

                  <dd className="mt-1 text-sam-fg">{detail.failure_reason ?? "—"}</dd>
                ...
                <div>
                  ...
                  <dd className="mt-1">{detail.admin_note ?? "—"}</dd>
              ...
              <p className="mt-4 text-sam-warning sam-text-xxs">배달 행 없음</p>
            ...
            <div className="mt-4 flex flex-wrap gap-2 border-t border-sam-border pt-3">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const reason = window.prompt("실패 사유", "admin_delivery_failed");
