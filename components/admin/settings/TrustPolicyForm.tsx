...
export function TrustPolicyForm({ values, onChange }: TrustPolicyFormProps) {
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-ui-rect border border-sam-border bg-signature/5 px-3 py-2.5 text-[12px] leading-relaxed text-sam-fg">
        <p className="font-medium text-sam-fg">배터리 % (화면 숫자)</p>
        <p className="mt-1 text-sam-muted">
          기본 데이터는 <code className="rounded bg-sam-surface/80 px-1 text-[11px]">profiles.trust_score</code>(0~100, 기본
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <label htmlFor="trustReviewEnabled" className="text-[14px] text-sam-fg">
          후기·신뢰도 사용
        </label>
      </div>
      <div className="flex items-center gap-2">
        <input
          ...
          className="rounded border-sam-border"
        />
        <label htmlFor="mannerScoreVisible" className="text-[14px] text-sam-fg">
          배터리 노출
        </label>
      </div>
      <div>
        <label htmlFor="speedDisplayLabel" className="block text-[14px] text-sam-fg">
          배터리 표시 라벨
        </label>
        <input
          type="text"
          ...
          placeholder="배터리"
          className="mt-1 w-full max-w-xs rounded border border-sam-border px-3 py-2 text-[14px]"
        />
