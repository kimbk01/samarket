    ...
    <div className="space-y-6">
      ...
      <AdminCard title="참조 SLO (p95·비율 — docs/messenger-performance-targets 와 동기)">
        <div className="grid gap-4 md:grid-cols-2">
          <pre className="overflow-x-auto rounded-ui-rect bg-sam-app p-4 text-[11px] leading-relaxed text-sam-fg">
      ...
      <AdminCard title="SLO 요약 (인메모리 윈도우 — 관측값 vs 목표)">
        {!summary?.sloDigest?.length ? (
          <p className="text-[14px] text-sam-muted">아직 집계할 샘플이 없습니다.</p>
        ) : (
              ...
              <tbody>
                {summary.sloDigest.map((row) => (
                  <tr key={row.id} className="border-b border-sam-border/60">
                    ...
                    <td className="py-2 pr-2 font-mono text-[11px] text-sam-muted">
                    ...
                    <td className="py-2 pr-2 font-mono text-[11px] text-sam-fg">
      ...
      <AdminCard title="성공/실패 누적 (구독·시그널링)">
        {!summary?.outcomeStats?.length ? (
          <p className="text-[14px] text-sam-muted">아직 outcome 이벤트가 없습니다.</p>
        ) : (
          <div className="overflow-x-auto">
              ...
              <tbody>
                {summary.outcomeStats.map((o) => (
                  <tr key={o.key} className="border-b border-sam-border/60">
                    <td className="py-2 pr-3 font-mono text-[11px] text-sam-muted">{o.key}</td>
