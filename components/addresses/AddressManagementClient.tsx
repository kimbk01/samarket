...
export function AddressManagementClient() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!a.ok || !aj.ok) {
        setLoadErr(typeof aj.error === "string" ? aj.error : "목록을 불러오지 못했어요.");
        return;
      }
  ...
  async function patchDefaults(id: string, patch: Record<string, boolean>) {
    ...
    try {
      ...
      if (!res.ok || !j.ok) {
        alert(typeof j.error === "string" ? j.error : "기본값 변경 실패");
        return;
      }
      await load();
    } finally {
  ...
  async function removeRow(id: string) {
    ...
    try {
      ...
      const j = (await res.json()) as { ok?: boolean; error?: string };
      if (!res.ok || !j.ok) {
        alert(typeof j.error === "string" ? j.error : "삭제 실패");
        return;
      }
      await load();
    } finally {
  ...
  return (
    <div className="min-h-screen bg-background">
      <MySubpageHeader
        title="주소 관리"
        subtitle="생활·거래·배달 기본 주소"
        backHref="/mypage"
        section="orders"
      />
      <div className="mx-auto max-w-lg space-y-5 px-4 py-4 pb-28 md:max-w-3xl">
        <p className="text-[13px] leading-relaxed text-gray-600">
          생활·동네, 중고거래, 배달에 쓰는 주소를 한곳에서 관리해요. 대표 주소와 용도별 기본값을 각각 지정할 수
          있어요.
        </p>
        ...
        <div>
          <div className="mb-2 flex items-center justify-between">
            <h2 className="text-[15px] font-semibold text-gray-900">저장된 주소</h2>
            <span className="text-[11px] text-gray-500">라디오: 대표 주소</span>
          </div>
