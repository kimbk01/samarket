        ...
        <section className="mt-4 rounded-ui-rect border border-sam-border-soft bg-sam-surface p-4 shadow-sm" id="comments">
          ...
          <ul className="mt-3 divide-y divide-sam-border-soft">
            {comments.map((c) => (
              <li key={c.id} className="py-3">
                <p className="text-[12px] font-medium text-sam-muted">{c.author_name}</p>
                <p className="mt-1 text-[14px] text-sam-fg">{c.content}</p>
          ...
          <form onSubmit={onSubmitComment} className="mt-4 border-t border-sam-border-soft pt-4">
            <textarea
              ...
              disabled={!me?.id || busy}
              rows={3}
              className="w-full rounded-ui-rect border border-sam-border px-3 py-2 text-[14px] outline-none focus:border-sam-border"
            />
            <button
              type="submit"
