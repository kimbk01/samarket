  ...
  return (
    <section className="mt-4">
      <div className={CM_FEED_CARD_CLASS}>
        <h2 className="m-0 text-[17px] font-bold leading-[1.35] text-[var(--cm-text)]">{t("community_similar_posts")}</h2>
            ...
            <li key={p.id} className="py-3.5 first:pt-0">
              <Link href={philifeAppPaths.post(p.id)} className="flex min-w-0 gap-2.5 active:opacity-90">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 line-clamp-1 text-[15px] font-semibold leading-[1.4] text-[var(--cm-text)]">{p.title || t("community_no_title")}</p>
                  {preview ? (
                    <p className="mt-0.5 line-clamp-1 text-[13px] font-normal leading-[1.45] text-[var(--cm-text-muted)]">{preview}</p>
                  ) : null}
                  <div className="mt-1.5 flex min-w-0 items-center justify-between gap-2">
                    <p className={`min-w-0 flex-1 truncate ${CM_META_CLASS}`}>
                      {p.location_label}
                    ...
                    <div className="flex shrink-0 items-center gap-2 text-[12px] font-normal text-[var(--cm-text-muted)]">
