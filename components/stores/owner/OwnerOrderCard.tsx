...
export function OwnerOrderCard({
  ...
  return (
    <article className="rounded-ui-rect border border-gray-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <p className="font-mono text-xs font-semibold text-gray-500">{order.order_no}</p>
      ...
      <p className="mt-2 line-clamp-2 text-sm text-gray-600">{menuSummary(order.items)}</p>
      <p className="mt-2 text-lg font-bold text-gray-900">{formatMoneyPhp(order.total_amount)}</p>

      <div className="mt-3 flex gap-2">
        <Link
          href={detailHref}
          className="flex-1 rounded-ui-rect bg-gray-100 py-2.5 text-center text-sm font-semibold text-gray-800"
        >
          상세보기
        </Link>
        <Link
          href={`/my/business/store-order-chat/${encodeURIComponent(order.id)}`}
          className="flex-1 rounded-ui-rect border border-gray-200 bg-signature/5 py-2.5 text-center text-sm font-semibold text-gray-900"
        >
          고객 문의
        </Link>
      </div>
