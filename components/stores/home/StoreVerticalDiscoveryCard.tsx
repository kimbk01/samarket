...
function statusBadge(status: BrowseStoreListItem["status"]) {
  if (status === "open") {
    return (
      <span className="shrink-0 rounded-md bg-[#E7F7EC] px-2 py-0.5 text-[12px] font-semibold text-[#31A24C] dark:bg-[#1F3528] dark:text-[#5CD67C]">
        영업중
      </span>
    );
  }
  if (status === "preparing") {
    return (
      <span className="shrink-0 rounded-md bg-[#FFF8E7] px-2 py-0.5 text-[12px] font-semibold text-[#B78100] dark:bg-[#3D3420] dark:text-[#F5C842]">
        준비중
      </span>
    );
  }
  return (
    <span className="shrink-0 rounded-md bg-[#E4E6EB] px-2 py-0.5 text-[12px] font-semibold text-[#65676B] dark:bg-[#3A3B3C] dark:text-[#B0B3B8]">
      휴무
    </span>
  );
}
      ...
      <Link href={storeHref} className="block active:bg-[#F2F3F5] dark:active:bg-[#2F3031]">
        <div className="relative aspect-[5/3] w-full overflow-hidden bg-[#F0F2F5] dark:bg-[#3A3B3C]">
          ...
          <div className="absolute left-2 top-2 flex flex-wrap gap-1">
            {store.isFeatured ?
              <span className="rounded-md bg-white/95 px-2 py-0.5 text-[11px] font-semibold text-[#1877F2] shadow-sm dark:bg-[#242526]/95 dark:text-[#4599FF]">
                추천
              </span>
            : null}
            {adHint ?
              <span className="rounded-md bg-white/95 px-2 py-0.5 text-[11px] font-semibold text-[#050505] shadow-sm dark:bg-[#242526]/95 dark:text-[#E4E6EB]">
                {adHint}
              </span>
            : null}
          </div>
          ...
          {flags.length > 0 ?
            <div className="flex flex-wrap gap-1">
              {flags.map((f) => (
                <span
                  key={f}
                  className="rounded-md bg-[#F0F2F5] px-2 py-0.5 text-[11px] font-semibold text-[#65676B] dark:bg-[#3A3B3C] dark:text-[#B0B3B8]"
                >
                  {f}
                </span>
              ))}
          ...
          {store.featuredItems.slice(0, 3).map((it) => (
            <li key={it.productId}>
              <Link
                ...
                className={`flex justify-between gap-2 rounded-md py-1.5 text-[13px] active:bg-[#F0F2F5] dark:active:bg-[#3A3B3C]`}
