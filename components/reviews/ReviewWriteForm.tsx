  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-4">
        ...
        <div className="flex gap-1">
          {[1, 2, 3, 4, 5].map((r) => (
            <button
              ...
              type="button"
              onClick={() => setRating(r)}
              className={`rounded border px-2 py-1 sam-text-body ${
                rating >= r
        ...
        <div className="flex flex-wrap gap-2">
          {REVIEW_TAGS.map((tag) => (
            <button
              ...
              className={`rounded-full border px-3 py-1.5 sam-text-helper ${
                tags.includes(tag)
                  ? "border-signature bg-signature/10 text-signature"
      ...
      <div className="flex gap-2">
        ...
        <button
          type="submit"
          className="flex-1 rounded-ui-rect bg-signature py-2.5 sam-text-body font-medium text-white"
        >
          후기 남기기
        </button>
      </div>
