  ...
  blocks.forEach((block, blockIdx) => {
    ...
    if (block.type === "date") {
      items.push(
        <li key={`date-${block.dateKey}`} className={`flex justify-center ${ig ? "py-5" : "py-4"}`}>
          {ig ? (
            <span className="rounded-full bg-black/[0.05] px-3.5 py-1.5 text-[12px] font-medium leading-none text-[#8E8E8E]">
              {block.label}
            </span>
          ) : (
          ...
          <li key={block.msg.id} className={`flex justify-start ${gapFromPrev}`}>
            <div className={`flex ${IG_DM_BUBBLE_ROW_MAX} items-end gap-2.5`}>
              <div className="flex shrink-0 flex-col justify-end">
                <div className="relative h-8 w-8 overflow-hidden rounded-full bg-gradient-to-br from-[#E1306C]/18 to-[#F77737]/18 ring-1 ring-black/[0.06]">
                  {partnerAvatar ? (
                    ...
                    <div className="flex h-full w-full items-center justify-center text-[13px] font-semibold text-[#262626]">
              ...
              <div className="flex min-w-0 flex-1 flex-col">
                ...
                >
                  <p className={`w-full whitespace-pre-wrap break-words text-left ${IG_DM_BODY_TEXT} text-[#262626]`}>
                    {body}
                  </p>
                </div>
                <div className="mt-0.5 flex justify-start">
                  <span className="text-[11px] font-normal tabular-nums leading-4 text-[#A8A8A8]">
    ...
    if (block.type === "opponent") {
      ...
      const opponentText = ig
        ? `${IG_DM_BODY_TEXT} text-left text-[#262626]`
            ...
            <div className="flex shrink-0 flex-col justify-end">
              ...
              >
                {partnerAvatar ? (
                  ...
                  <div
                    className={`flex h-full w-full items-center justify-center ${ig ? "text-[13px] font-semibold text-[#262626]" : "text-[14px] font-medium text-gray-500"}`}
                  >
                    {(partnerNickname || "?").charAt(0)}
                  </div>
                )}
                  ...
                  <div key={msg.id} className={i === 0 ? "" : ig ? "mt-0.5" : "mt-1"}>
                    {emojiOnly ? (
                      ...
                      >
                        {hasReply && (
                          <div
                            className={`mb-1 border-l-2 pl-2 ${ig ? "border-black/12 text-left text-[12px] font-normal leading-[1.35] text-[#8E8E8E]" : "text-[12px] border-gray-300 text-[#999999]"}`}
                          >
                    ...
                    {showTime && (
                      <div className={`flex justify-end ${ig ? "mt-0.5" : "mt-1"}`}>
                        <span
                          className={`text-[11px] font-normal tabular-nums ${ig ? "leading-4 text-[#A8A8A8]" : "leading-[14px] text-[#999999]"}`}
                        >
    ...
    if (block.type === "mine") {
      const msgs = block.messages;
      const mineBubbleShell = ig ? "bg-[#0095F6] shadow-none" : "bg-[#FEE500] shadow-sm";
      const mineText = ig
                ...
                <div key={msg.id} className={i === 0 ? "" : ig ? "mt-0.5" : "mt-1"}>
                  ...
                  {showTime && (
                    <div className={`flex items-center justify-start gap-1 ${ig ? "mt-0.5" : "mt-1"}`}>
                      {msg.isRead && (
                        <span
                          className={`text-[11px] font-normal ${ig ? "leading-4 text-[#A8A8A8]" : "leading-[14px] text-[#999999]"}`}
                        >
                          읽음
                        </span>
                      )}
