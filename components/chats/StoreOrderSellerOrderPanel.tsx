...
export function StoreOrderSellerOrderPanel({
  ...
  const sendBlock = (
    <div className="shrink-0 border-b border-[#EFEFEF] px-3 py-3">
      <button
        ...
        className="w-full rounded-xl bg-[#0095F6] px-4 py-3 text-[15px] font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40"
      >
      ...
      {sendToast ? (
        <p className="mt-2 text-center text-[13px] text-[#8E8E8E]">{sendToast}</p>
      ) : null}
    </div>
  );

  const orderPreviewBlock = (
    <div className="px-3 py-3 text-[14px] text-[#262626]">
      {loading ? (
        <p className="text-center text-[#8E8E8E]">주문 불러오는 중…</p>
      ) : loadErr ? (
        <p className="text-center text-red-600">{loadErr}</p>
      ) : orderSnap ? (
        <pre className="whitespace-pre-wrap break-words font-sans text-[13px] leading-[1.45] text-[#262626]">
  ...
  const scrollBody = (
    <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-3 text-[14px] text-[#262626]">
      {loading ? (
        <p className="text-center text-[#8E8E8E]">주문 불러오는 중…</p>
      ) : loadErr ? (
        <p className="text-center text-red-600">{loadErr}</p>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          <div className="border-b border-[#EFEFEF] px-3 py-3">
            ...
            <p className="mb-2 text-[12px] leading-snug text-[#8E8E8E]">
              <span className="font-mono text-[11px] text-[#A8A8A8]">/my/business</span>
              <span className="mx-1 text-[#DBDBDB]">·</span>
              매장 관리 허브와 동일한 메뉴입니다.
            </p>
            <Link
              href={businessHubHref}
              onClick={() => onOpenChange(false)}
              className="flex w-full items-center justify-center rounded-xl bg-[#262626] px-4 py-3 text-[15px] font-semibold text-white hover:bg-black"
            >
              내 상점 관리로 이동
            </Link>
            <div className="mt-3">
              <MyBusinessNavList
                ...
                className="rounded-xl border border-[#EFEFEF] bg-[#FAFAFA] pb-2 shadow-none"
              />
            </div>
          </div>
          <div className="border-t border-[#EFEFEF] px-0 pt-1">
