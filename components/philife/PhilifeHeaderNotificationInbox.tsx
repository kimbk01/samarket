...
export function PhilifeHeaderNotificationInbox() {
  ...
  const panel = !domReady || !open
    ? null
    : createPortal(
        <>
          <div
            className="fixed inset-0 z-[100] touch-none bg-sam-surface/40 backdrop-blur-[1px] transition-opacity"
            role="presentation"
            aria-hidden
          ...
          <div
            id={panelId}
            className="fixed left-0 right-0 top-12 z-[110] flex max-h-[min(75dvh,32rem)] flex-col overflow-hidden border-b border-sam-border/90 bg-sam-surface/98 shadow-[0_10px_40px_-12px_rgba(0,0,0,0.2)]"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="flex shrink-0 items-center justify-between gap-2 border-b border-sam-border/80 px-3 py-2.5">
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                ...
                <Link
                  ...
                  onClick={() => setOpen(false)}
                  className="sam-header-action flex h-9 w-9 items-center justify-center rounded-full text-sam-fg"
            ...
            <div
              className={`min-h-0 flex-1 overflow-y-auto overscroll-contain py-2 ${APP_MAIN_GUTTER_X_CLASS}`}
            >
              {loading ? (
