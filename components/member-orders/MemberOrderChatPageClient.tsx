...
export function MemberOrderChatPageClient({ orderId }: { orderId: string }) {
  ...
  if (!buyerId) {
    return (
      <div className="min-h-screen bg-sam-app px-4 py-16 text-center">
        <p className="text-sm text-sam-muted">{t("member_order_chat_member_required")}</p>
  ...
  if (!order || order.buyer_user_id !== buyerId) {
    return (
      <div className="min-h-screen bg-sam-app px-4 py-16 text-center">
        <p className="text-sm text-sam-muted">{t("member_order_chat_open_failed")}</p>
        <Link href={BASE} className="mt-4 inline-block text-signature underline">
          {t("member_order_chat_list_link")}
        </Link>
      </div>
  ...
  if (isStoreOrderChatDisabledForBuyer(order.order_status)) {
    return (
      <div className="min-h-screen bg-sam-app px-4 py-16 text-center">
        <p className="text-sm text-sam-muted">{t("member_order_chat_cancelled_blocked")}</p>
        <Link
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#e8e6ef]">
      ...
      {toast ? (
        <p className="bg-sam-ink px-3 py-2 text-center text-xs text-white">{toast}</p>
      ) : null}
