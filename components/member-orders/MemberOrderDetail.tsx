...
export function MemberOrderDetail({
  ...
  return (
    <div className="min-h-screen bg-sam-app pb-36">
      <header className="sticky top-0 z-20 border-b border-sam-border-soft bg-sam-surface px-3 py-3">
      ...
      <div className="mx-auto max-w-lg space-y-3 px-3 py-4">
        ...
        <section className="rounded-ui-rect bg-sam-surface p-4 shadow-sm ring-1 ring-sam-border-soft">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              <p className="text-lg font-bold text-sam-fg">{order.store_name}</p>
              <Link
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <Link
                ...
                className="inline-flex items-center gap-1 rounded-full bg-signature/5 px-3 py-1.5 text-xs font-bold text-sam-fg ring-1 ring-sam-border"
              >
          ...
          <p className="mt-2 font-mono text-xs text-sam-meta">{order.order_no}</p>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <span className="rounded-ui-rect bg-sam-surface-muted px-2 py-0.5 text-xs font-medium text-sam-fg">
              {t("member_order_payment")} {tt(MEMBER_PAYMENT_LABEL[order.payment_status])}
            </span>
          </div>
          <p className="mt-3 text-sm font-medium text-sam-fg">{tt(MEMBER_STATUS_USER_MESSAGE[order.order_status])}</p>
        ...
        <section className="rounded-ui-rect bg-sam-surface p-4 shadow-sm ring-1 ring-sam-border-soft">
          <h2 className="text-sm font-bold text-sam-fg">{t("member_order_menu_title")}</h2>
          <div className="mt-3">
        ...
        <section className="rounded-ui-rect bg-sam-surface p-4 shadow-sm ring-1 ring-sam-border-soft">
          <h2 className="text-sm font-bold text-sam-fg">{t("member_order_amount_title")}</h2>
          <div className="mt-3">
        ...
        {order.order_type === "delivery" ? (
          <section className="rounded-ui-rect bg-sam-surface p-4 shadow-sm ring-1 ring-sam-border-soft">
            <h2 className="text-sm font-bold text-sam-fg">{t("member_order_delivery_info_title")}</h2>
            <dl className="mt-3 space-y-2 text-sm">
              <div>
                <dt className="text-xs text-sam-muted">{t("member_order_address")}</dt>
                <dd className="text-sam-fg">{order.delivery_address_summary ?? "—"}</dd>
              </div>
              <div>
                <dt className="text-xs text-sam-muted">{t("member_order_contact")}</dt>
                <dd className="font-mono text-sam-fg">{order.buyer_phone}</dd>
              </div>
              <div>
                <dt className="text-xs text-sam-muted">{t("member_order_request_note")}</dt>
                <dd className="text-sam-fg">{order.request_message?.trim() || "—"}</dd>
              </div>
            </dl>
          </section>
        ) : (
          <section className="rounded-ui-rect bg-sam-surface p-4 shadow-sm ring-1 ring-sam-border-soft">
            <h2 className="text-sm font-bold text-sam-fg">{t("member_order_pickup_title")}</h2>
            <dl className="mt-3 space-y-2 text-sm">
              <div>
                <dt className="text-xs text-sam-muted">{t("member_order_pickup_guide")}</dt>
                <dd className="text-sam-fg">{order.pickup_note ?? "—"}</dd>
              </div>
              <div>
                <dt className="text-xs text-sam-muted">{t("member_order_request_note")}</dt>
                <dd className="text-sam-fg">{order.request_message?.trim() || "—"}</dd>
              ...
              <div>
                <dt className="text-xs text-sam-muted">{t("member_order_pickup_available")}</dt>
        ...
        <section className="rounded-ui-rect bg-sam-surface p-4 shadow-sm ring-1 ring-sam-border-soft">
          <h2 className="text-sm font-bold text-sam-fg">{t("member_order_status_history")}</h2>
          <div className="mt-4">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 border-t border-sam-border-soft bg-sam-surface/95 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] shadow-[0_-4px_20px_rgba(0,0,0,0.06)] backdrop-blur">
        <div className="mx-auto flex max-w-lg flex-col gap-2">
          {!issueState ? (
            <Link
              href={`${listHref}/${encodeURIComponent(order.id)}/chat`}
              className="flex w-full items-center justify-center gap-2 rounded-ui-rect border border-sam-border bg-signature/5 py-3 text-sm font-bold text-sam-fg"
            >
          ...
          {canCancelRequest ? (
            <button
              ...
              className="w-full rounded-ui-rect border border-red-200 bg-sam-surface py-3 text-sm font-bold text-red-700"
            >
          ...
          {completed ? (
            <>
              <button
                ...
                className="w-full rounded-ui-rect bg-sam-ink py-3 text-sm font-bold text-white"
              >
              ...
              <button
                ...
                className="w-full rounded-ui-rect border border-sam-border bg-sam-surface py-3 text-sm font-semibold text-sam-fg"
              >
          ...
          {issueState && !completed ? (
            <p className="py-2 text-center text-xs text-sam-muted">
