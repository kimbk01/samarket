...
function MeetingHomeFullSummary({
  ...
  return (
    <div className="rounded-ui-rect border border-sam-border-soft bg-sam-surface px-4 py-3.5 shadow-sm">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 sam-text-body-secondary text-sam-muted">
    ...
    <div className="space-y-3 pb-24 pt-1">
      ...
      {meeting.welcome_message ? (
        <div className="flex items-start gap-3 rounded-ui-rect bg-emerald-50 px-4 py-3">
          <span className="mt-0.5 sam-text-page-title">🙌</span>
          <p className="sam-text-body-secondary leading-relaxed text-emerald-900">{meeting.welcome_message}</p>
        </div>
      ) : null}
            ...
            <div className="flex items-center gap-3 px-1">
              <div className="flex -space-x-2.5">
                ...
                {joinedMembers.length > 7 && (
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-sam-border-soft sam-text-xxs font-semibold text-sam-muted ring-2 ring-sam-surface">
                    +{joinedMembers.length - 7}
                  </div>
                )}
              </div>
          ...
          {onTabChange && (
            <div className="grid grid-cols-5 gap-1">
              {NAV_ACTIONS.map(({ tab, emoji, label }) => (
                ...
                >
                  <span className="sam-text-hero">{emoji}</span>
                  <span className="sam-text-xxs font-medium text-sam-muted">{label}</span>
                </button>
              ))}
            </div>
          )}
            ...
            <div className="min-w-0 flex-1">
              {latestNotice ? (
                <>
                  ...
                  <p className="mt-1 sam-text-xxs text-amber-600/80">
      ...
      {compactSummary && joinedMembers.length > 0 && onTabChange ? (
        <div className="flex items-center justify-between rounded-ui-rect border border-sam-border/80 bg-sam-surface px-4 py-3.5 shadow-sm">
          ...
          >
            ...
            <span className="pl-1 sam-text-body-lg">›</span>
          </button>
        </div>
      ) : null}
