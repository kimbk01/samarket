...
type MyTestLoginSectionProps = {
  redirectTo?: string;
};

export function MyTestLoginSection({ redirectTo }: MyTestLoginSectionProps) {
  if (!isTestUsersSurfaceEnabled()) return null;

  return (
    <section className="space-y-2" aria-label="아이디 로그인">
      <h2 className="px-1 text-[13px] font-semibold text-[#8E8E8E]">아이디 로그인</h2>
      <div className="overflow-hidden rounded-xl border border-[#DBDBDB] bg-white">
        <TestLoginBar redirectTo={redirectTo} />
      </div>
      <p className="px-1 text-[11px] leading-relaxed text-[#8E8E8E]">
