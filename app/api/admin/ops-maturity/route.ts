...
import {
  loadOpsMaturityBundleFromDb,
  saveOpsMaturityBundleToDb,
} from "@/lib/ops-maturity/ops-maturity-db";
...
function isBundle(x: unknown): x is OpsMaturityBundleV1 {
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadOpsMaturityBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultOpsMaturityBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveOpsMaturityBundleToDb(sb, body.bundle);
    if (!saved.ok) {
