...
import {
  loadDevSprintsBundleFromDb,
  saveDevSprintsBundleToDb,
} from "@/lib/dev-sprints/dev-sprints-db";
...
function isBundle(x: unknown): x is DevSprintsBundleV1 {
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadDevSprintsBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultDevSprintsBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveDevSprintsBundleToDb(sb, body.bundle);
    if (!saved.ok) {
