...
import {
  loadLaunchWeekBundleFromDb,
  saveLaunchWeekBundleToDb,
} from "@/lib/launch-week/launch-week-db";
...
function isBundle(x: unknown): x is LaunchWeekBundleV1 {
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadLaunchWeekBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultLaunchWeekBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveLaunchWeekBundleToDb(sb, body.bundle);
    if (!saved.ok) {
