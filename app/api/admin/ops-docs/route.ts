...
import { loadOpsDocsBundleFromDb, saveOpsDocsBundleToDb } from "@/lib/ops-docs/ops-docs-db";
...
function isBundle(x: unknown): x is OpsDocsBundleV1 {
  if (x == null || typeof x !== "object") return false;
  const o = x as Record<string, unknown>;
  return o.version === 1 && Array.isArray(o.documents);
}
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadOpsDocsBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultOpsDocsBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveOpsDocsBundleToDb(sb, body.bundle);
    if (!saved.ok) {
