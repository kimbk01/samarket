...
import {
  loadOpsKnowledgeGraphBundleFromDb,
  saveOpsKnowledgeGraphBundleToDb,
} from "@/lib/ops-knowledge-graph/ops-knowledge-graph-db";
...
function isBundle(x: unknown): x is OpsKnowledgeGraphBundleV1 {
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadOpsKnowledgeGraphBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultOpsKnowledgeGraphBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveOpsKnowledgeGraphBundleToDb(sb, body.bundle);
    if (!saved.ok) {
