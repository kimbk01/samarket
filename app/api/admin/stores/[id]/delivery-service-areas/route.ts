...
async function loadStoreGeo(sb: NonNullable<ReturnType<typeof tryGetSupabaseForStores>>, storeId: string) {
  const { data, error } = await sb
    .from("stores")
    .select("id, city, region, lat, lng, delivery_radius_km, delivery_service_area_authority, store_name")
    .eq("id", storeId)
    .maybeSingle();
...
/** Admin — SAME SSOT as Owner delivery-service-areas. */
...
export async function PUT(
  ...
  try {
    ...
    const result = await replaceStoreDeliveryServiceAreas(sb, {
      storeId,
      source: "admin",
      activateV2: body.activateV2 === true,
      storeHomeLguId,
    ...
    return NextResponse.json({
      ...
      ...buildDeliveryServiceAreaEditorPayload(
        { ...store, delivery_service_area_authority: result.authority },
        selected
      ),
    });
  } catch (e) {
