...
async function removeAdminUploadedOrderMatchSoundIfPresent(
  ...
  if (!path || !path.startsWith("_admin/global-order-match-chat/")) return;
  const { error } = await sb.storage.from(SOUND_BUCKET).remove([path]);
  if (error) {
    console.warn("[removeAdminUploadedOrderMatchSoundIfPresent]", path, error.message);
  }
}

async function fetchOrderMatchAlertSoundUrl(
  sb: NonNullable<ReturnType<typeof tryGetSupabaseForStores>>
): Promise<string | null> {
  ...
  if (error) {
    ...
    console.warn("[fetchOrderMatchAlertSoundUrl]", error.message);
    return null;
  }
...
export async function GET() {
  ...
  if (error) {
    ...
    console.error("[GET order-match-chat-alert-sound]", error);
    return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
  }

  const url = parseOrderMatchAlertSoundUrl(row?.value_json);
...
export async function POST(req: NextRequest) {
  ...
  const path = `_admin/global-order-match-chat/${randomUUID()}.${ext}`;
  ...
  if (upErr) {
    console.error("[POST order-match-chat-alert-sound upload]", upErr);
  ...
  const { data: prevRow } = await sb
    ...
    .eq("key", ORDER_MATCH_CHAT_ALERT_SOUND_KEY)
    .maybeSingle();
  const prevUrl = parseOrderMatchAlertSoundUrl(prevRow?.value_json);
  if (prevUrl && prevUrl !== publicUrl) {
    await removeAdminUploadedOrderMatchSoundIfPresent(sb, prevUrl);
  }
  ...
  if (dbErr) {
    ...
    console.error("[POST order-match-chat-alert-sound upsert]", dbErr);
  ...
  void appendAuditLog(sb, {
    actor_type: "admin",
    actor_id: actorId,
    target_type: "order_match_chat_alert_sound",
    target_id: "global",
    action: "order_match_chat_alert_sound.upload",
...
export async function DELETE(req: NextRequest) {
  ...
  const prevUrl = parseOrderMatchAlertSoundUrl(row?.value_json);
  await removeAdminUploadedOrderMatchSoundIfPresent(sb, prevUrl);

  const { error } = await sb.from("admin_settings").delete().eq("key", ORDER_MATCH_CHAT_ALERT_SOUND_KEY);
  if (error) {
    if (error.message?.includes("admin_settings") && error.message.includes("does not exist")) {
      return NextResponse.json({ ok: false, error: "table_missing" }, { status: 503 });
    }
    console.error("[DELETE order-match-chat-alert-sound]", error);
    return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
  }
  ...
  void appendAuditLog(sb, {
    ...
    target_type: "order_match_chat_alert_sound",
    target_id: "global",
    action: "order_match_chat_alert_sound.delete",
    after_json: payload as unknown as Record<string, unknown>,
    ip: rm.ip,
    user_agent: rm.userAgent,
  });
...
export async function PUT(req: NextRequest) {
  ...
  if (raw === null || raw === undefined || String(raw).trim() === "") {
    ...
    if (error) {
      ...
      console.error("[PUT order-match-chat-alert-sound delete]", error);
      return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
    }
  } else {
    ...
    if (error) {
      ...
      console.error("[PUT order-match-chat-alert-sound upsert]", error);
      return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
    }
  }

  const url = await fetchOrderMatchAlertSoundUrl(sb);
  ...
  void appendAuditLog(sb, {
    ...
    target_type: "order_match_chat_alert_sound",
    target_id: "global",
    action: "order_match_chat_alert_sound.update",
    after_json: payload as unknown as Record<string, unknown>,
    ip: rm.ip,
    user_agent: rm.userAgent,
  });
