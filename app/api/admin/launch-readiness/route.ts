...
import {
  loadLaunchReadinessBundleFromDb,
  saveLaunchReadinessBundleToDb,
} from "@/lib/launch-readiness/launch-readiness-db";
...
function isBundle(x: unknown): x is LaunchReadinessBundleV1 {
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadLaunchReadinessBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultLaunchReadinessBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveLaunchReadinessBundleToDb(sb, body.bundle);
    if (!saved.ok) {
