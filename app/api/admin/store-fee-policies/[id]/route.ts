...
async function assertNoActiveConflict(sb: ReturnType<typeof tryGetSupabaseForStores>, opts: {
  ...
  const scope = opts.store_id ? "store" : opts.category_id ? "category" : "default";
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if ("store_id" in body) patch.store_id = body.store_id ?? null;
  ...
  // conflict check (apply next scope/window/priority/is_active)
  ...
  const wasArchived = Boolean((current as any).is_archived);
  ...
  const nextStoreId =
    "store_id" in patch ? (patch.store_id as string | null) : ((current as any).store_id as string | null);
  const nextCategoryId =
    "category_id" in patch ? (patch.category_id as string | null) : ((current as any).category_id as string | null);
  const nextIsActive =
    "is_active" in patch ? Boolean(patch.is_active) : Boolean((current as any).is_active);
  const nextStartsAt =
    "starts_at" in patch ? ((patch.starts_at as string | null) ?? null) : ((current as any).starts_at as string | null);
  const nextEndsAt =
    "ends_at" in patch ? ((patch.ends_at as string | null) ?? null) : ((current as any).ends_at as string | null);
  const nextPriority =
    "priority" in patch ? Math.max(0, Math.round(Number(patch.priority ?? 0))) : Math.max(0, Math.round(Number((current as any).priority ?? 0)));

  // 타입 규칙 재검증
  if (!nextStoreId && !nextCategoryId) {
    // default ok
  } else if (!nextStoreId && nextCategoryId) {
    // category ok
  } else if (nextStoreId) {
    // store ok
  } else {
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (Boolean((pre as any).is_archived)) {
    return NextResponse.json({ ok: true });
  }
