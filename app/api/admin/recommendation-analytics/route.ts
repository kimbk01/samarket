...
import {
  loadRecommendationAnalyticsBundleFromDb,
  saveRecommendationAnalyticsBundleToDb,
} from "@/lib/recommendation-analytics/recommendation-analytics-db";
...
function isBundle(x: unknown): x is RecommendationAnalyticsBundleV1 {
  if (x == null || typeof x !== "object") return false;
  const o = x as Record<string, unknown>;
  return o.version === 1 && Array.isArray(o.impressions);
}
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadRecommendationAnalyticsBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultRecommendationAnalyticsBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveRecommendationAnalyticsBundleToDb(sb, body.bundle);
    if (!saved.ok) {
