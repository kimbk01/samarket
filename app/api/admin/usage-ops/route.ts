...
import {
  loadUsageOpsBundleFromDb,
  saveUsageOpsBundleToDb,
} from "@/lib/usage/usage-ops-db";
...
function isBundle(x: unknown): x is UsageOpsBundleV1 {
  if (x == null || typeof x !== "object") return false;
  const o = x as Record<string, unknown>;
  return o.version === 1 && Array.isArray(o.usageMetrics);
}
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadUsageOpsBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultUsageOpsBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveUsageOpsBundleToDb(sb, body.bundle);
    if (!saved.ok) {
