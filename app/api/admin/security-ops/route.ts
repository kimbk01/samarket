...
import {
  loadSecurityOpsBundleFromDb,
  saveSecurityOpsBundleToDb,
} from "@/lib/security/security-db";
...
function isBundle(x: unknown): x is SecurityOpsBundleV1 {
  if (x == null || typeof x !== "object") return false;
  const o = x as Record<string, unknown>;
  return o.version === 1 && Array.isArray(o.checks) && Array.isArray(o.issues);
}
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadSecurityOpsBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultSecurityOpsBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveSecurityOpsBundleToDb(sb, body.bundle);
    if (!saved.ok) {
