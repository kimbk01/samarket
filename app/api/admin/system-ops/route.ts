...
import {
  loadSystemOpsBundleFromDb,
  saveSystemOpsBundleToDb,
} from "@/lib/system/system-ops-db";
...
function isBundle(x: unknown): x is SystemOpsBundleV1 {
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadSystemOpsBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultSystemOpsBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveSystemOpsBundleToDb(sb, body.bundle);
    if (!saved.ok) {
