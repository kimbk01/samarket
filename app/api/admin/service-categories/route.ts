...
import {
  loadServiceCategoriesBundleFromDb,
  saveServiceCategoriesBundleToDb,
} from "@/lib/admin-settings/service-categories-db";
...
function isBundle(x: unknown): x is ServiceCategoriesBundleV1 {
  if (x == null || typeof x !== "object") return false;
  const o = x as Record<string, unknown>;
  return o.version === 1 && Array.isArray(o.categories) && Array.isArray(o.subcategories);
}
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadServiceCategoriesBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultServiceCategoriesBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveServiceCategoriesBundleToDb(sb, body.bundle);
    if (!saved.ok) {
