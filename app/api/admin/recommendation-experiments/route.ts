...
import {
  loadRecommendationExperimentsBundleFromDb,
  saveRecommendationExperimentsBundleToDb,
} from "@/lib/recommendation-experiments/recommendation-experiments-db";
...
function isBundle(x: unknown): x is RecommendationExperimentsBundleV1 {
  if (x == null || typeof x !== "object") return false;
  const o = x as Record<string, unknown>;
  return o.version === 1 && Array.isArray(o.feedVersions) && Array.isArray(o.experiments);
}
  ...
  try {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultRecommendationExperimentsBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveRecommendationExperimentsBundleToDb(sb, body.bundle);
    if (!saved.ok) {
