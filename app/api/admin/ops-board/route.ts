...
import {
  loadOpsBoardBundleFromDb,
  saveOpsBoardBundleToDb,
} from "@/lib/ops-board/ops-board-db";
...
function isBundle(x: unknown): x is OpsBoardBundleV1 {
  if (x == null || typeof x !== "object") return false;
  const o = x as Record<string, unknown>;
  return o.version === 1 && Array.isArray(o.checklistTemplates);
}
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadOpsBoardBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultOpsBoardBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveOpsBoardBundleToDb(sb, body.bundle);
    if (!saved.ok) {
