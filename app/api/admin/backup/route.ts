...
import { loadBackupBundleFromDb, saveBackupBundleToDb } from "@/lib/backup/backup-db";
...
function isBundle(x: unknown): x is BackupBundleV1 {
  if (x == null || typeof x !== "object") return false;
  const o = x as Record<string, unknown>;
  return o.version === 1 && Array.isArray(o.snapshots);
}
...
export async function GET() {
  ...
  try {
    ...
    const loaded = await loadBackupBundleFromDb(sb);
    if (loaded.ok) {
    ...
    if (loaded.reason === "missing_table") {
      return NextResponse.json({
        ok: true,
        bundle: createDefaultBackupBundle(),
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const sb = getSupabaseServer();
    const saved = await saveBackupBundleToDb(sb, body.bundle);
    if (!saved.ok) {
