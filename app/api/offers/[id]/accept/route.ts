...
import { acceptPriceOffer } from "@/lib/offers/offers.service";
...
export async function POST(
  ...
  const result = await acceptPriceOffer(typedSb, {
  ...
  return NextResponse.json({
    ...
    chatRoomId: result.value.chatRoomId,
    chatRoomSource: result.value.chatRoomSource,
    messengerRoomId: result.value.messengerRoomId,
  });
}
