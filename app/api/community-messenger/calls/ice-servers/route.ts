...
export async function GET() {
  ...
  const turnUrls = parseUrls(process.env.COMMUNITY_MESSENGER_TURN_URLS);
  const turnUsername = process.env.COMMUNITY_MESSENGER_TURN_USERNAME?.trim() ?? "";
  const turnCredential = process.env.COMMUNITY_MESSENGER_TURN_CREDENTIAL?.trim() ?? "";
