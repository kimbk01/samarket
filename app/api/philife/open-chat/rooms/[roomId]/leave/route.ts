...
import { leaveOpenChatRoom, type OpenChatErrorCode } from "@/lib/open-chat/service";
...
export async function POST(_req: NextRequest, ctx: Ctx) {
  ...
  try {
    const result = await leaveOpenChatRoom(sb, {
      roomId: id,
      userId: auth.userId,
    });
    ...
    return NextResponse.json({ ok: true, roomId: result.data.roomId });
  } catch (error) {
