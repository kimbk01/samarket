...
import { joinOpenChatRoom, type OpenChatErrorCode } from "@/lib/open-chat/service";
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  let body: { nickname?: string | null; requestMessage?: string | null; inviteCode?: string | null } = {};
  try {
    body = await req.json();
  } catch {
    body = {};
  ...
  try {
    const result = await joinOpenChatRoom(sb, {
      roomId: id,
      userId: auth.userId,
    ...
    return NextResponse.json({
      ...
      chatRoomId: result.data.room.linkedChatRoomId,
    });
  } catch (error) {
