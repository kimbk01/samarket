...
import { createOpenChatNotice, type OpenChatErrorCode } from "@/lib/open-chat/service";
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  let body: { title?: string; body?: string; visibility?: "members" | "public" };
  try {
    body = await req.json();
  } catch {
  ...
  const result = await createOpenChatNotice(sb, {
    roomId: id,
