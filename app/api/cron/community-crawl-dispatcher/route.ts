...
/**
 * Community Crawl Dispatcher — due boards only → runCommunityCrawlBoard.
 */
async function runDispatcher(req: Request) {
  ...
  for (const row of due ?? []) {
    ...
    try {
      ...
      const crawl = await runCommunityCrawlBoard({
        sb,
        board,
        source,
        runKind: "SCHEDULED",
      ...
      results.push({
        ...
        published: crawl.publishedCount,
        alreadyPublished: crawl.alreadyPublishedCount,
