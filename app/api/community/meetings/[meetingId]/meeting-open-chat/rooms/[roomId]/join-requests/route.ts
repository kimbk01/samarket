...
import { listPendingMeetingOpenChatJoinRequests } from "@/lib/meeting-open-chat/ops-service";
...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  const list = await listPendingMeetingOpenChatJoinRequests(loaded.ctx.sb, rid);
  if (!list.ok) {
