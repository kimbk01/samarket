...
function OwnerCareCsInquiriesInner() {
  const sp = useSearchParams();
  const storeId = sp.get("storeId");
  const listBasePath = OwnerRoutes.customerCareCsInquiries(storeId).split("?")[0]!;
  return <MemberCsNoteListClient kind="inquiry" listBasePath={listBasePath} hideChrome />;
}

export default function OwnerCustomerCareCsInquiriesPage() {
  return (
    <Suspense
      fallback={
