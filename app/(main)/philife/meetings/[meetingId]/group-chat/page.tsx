...
import { CommunityGroupChatListClient } from "@/components/community-group-chat/CommunityGroupChatListClient";
...
import { loadCommunityGroupChatListInitialData } from "@/lib/community-group-chat/meeting-group-chat-list-initial-data";
...
export default async function MeetingGroupChatHubPage({ params }: Props) {
  ...
  if (isJoined && activeOpenChatRoomCount === 1 && defaultOpenChatRoomId) {
    redirect(philifeAppPaths.meetingGroupChatRoom(meeting.id, defaultOpenChatRoomId));
  }

  const initialListData = isJoined ? await loadCommunityGroupChatListInitialData(meeting.id) : null;
