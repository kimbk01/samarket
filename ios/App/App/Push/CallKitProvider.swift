...
final class CallKitProvider: NSObject, CXProviderDelegate {
  ...
  /**
   * PushKit orphan terminal VoIP — satisfy `reportNewIncomingCall` without Runtime registration,
   * then end immediately. Does not call `reconcileStaleSessionsBeforeIncoming` or register
   * Native Voice/Video sessions (unlike `reportIncomingCall`).
   */
  func reportOrphanTerminalVoipPushAndEnd(
    sessionId: String,
    handle: String,
    hasVideo: Bool,
    logDetail: String,
    ...
    if let existingUuid = callUuidBySessionId[sid] {
      DibayCallLog.infoCallV4(
        "ios_voip_terminal_orphan_dedup_end",
        callId: sid,
        owner: "terminal",
        reason: logDetail
      )
      provider.reportCall(with: existingUuid, endedAt: Date(), reason: .failed)
    ...
    hasVideoBySessionId[sid] = hasVideo
    ...
    provider.reportNewIncomingCall(with: uuid, update: update) { [weak self] error in
      ...
      if let error = error {
        DibayCallLog.infoCallV4(
          "ios_voip_terminal_orphan_report_failed",
          callId: sid,
          owner: "terminal",
          reason: error.localizedDescription
        )
        self.callUuidBySessionId.removeValue(forKey: sid)
        self.hasVideoBySessionId.removeValue(forKey: sid)
        self.outgoingSessionIds.remove(sid)
        completion()
        return
      }
      ...
      self.outgoingSessionIds.remove(sid)
      DibayCallLog.infoCallV4(
        "ios_voip_terminal_orphan_reported_and_ended",
        callId: sid,
        owner: "terminal",
        reason: logDetail
      )
  ...
  /**
   * Terminal dismiss lookup — returns only a registry-stored UUID.
   * DO NOT synthesize UUID(sessionId) here; unknown sessions must no-op.
   */
  func trackedCallKitUuid(sessionId: String) -> UUID? {
    let sid = sessionId.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !sid.isEmpty else { return nil }
    return callUuidBySessionId[sid]
  }

  /**
   * Canonical terminal authority end — tracked incoming CallKit only.
   * Runs runtime remote-terminal cleanup when Native owns the session, then removes registry.
   */
  func endTrackedIncomingCallKitSession(
    sessionId: String,
    reason: CXCallEndedReason,
    logDetail: String
  ) {
    let sid = sessionId.trimmingCharacters(in: .whitespacesAndNewlines)
    guard let uuid = callUuidBySessionId[sid] else {
      DibayCallLog.info(
        "ios_terminal_callkit_already_ended",
        sessionId: sid,
        detail: "reason=\(logDetail)_registry_empty"
      )
      return
    }
    if isOutgoingSession(sid) {
      DibayCallLog.info(
        "ios_terminal_stale_dropped",
        sessionId: sid,
        detail: "reason=outgoing_guard_endTracked source=\(logDetail)"
      )
      return
    }
    ...
    DibayCallLog.info(
      "ios_native_voice_callkit_end",
      sessionId: sid,
      detail: "reason=\(logDetail) uuid=\(uuid.uuidString)"
    )
    provider.reportCall(with: uuid, endedAt: Date(), reason: reason)
    callUuidBySessionId.removeValue(forKey: sid)
    hasVideoBySessionId.removeValue(forKey: sid)
