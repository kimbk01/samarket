...
export function buildMyBusinessNavGroups(ctx: MyBusinessNavContext): MyBusinessNavGroup[] {
  ...
  if (approved) {
    ...
    menuItems.push({
      label: "메뉴 관리",
      icon: "menu_board",
      href: `/my/business/menu?${q}`,
    });
    menuItems.push({
      label: "상품 등록",
      icon: "product",
      href: `/my/business/products/new?${q}${canSell ? "" : "&draft=1"}`,
    });
  }
  ...
  groups.push({
    ...
    items: [
      ...
      { label: "리뷰 관리", icon: "review", disabled: true, hint: "준비 중" },
      { label: "광고·프로모션", icon: "promo", disabled: true, hint: "준비 중" },
    ],
  });
