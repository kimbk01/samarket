...
function lastVisibleMarkableMessageId(
  ...
  if (!root) return null;
  const list = roomMessages.length > 0 ? roomMessages : snapshotMessages ?? [];
  return resolveVisibleRangeReadCursor({
    messages: list,
    previousCursorId: afterMessageId,
    isVisible: (messageId) => isLatestMessageVisibleEnoughInViewport(root, messageId),
  });
}
...
export function useMessengerRoomOpenMarkReadEffect(args: {
  ...
  /** mark_read 성공 후 notification_events room-read — 방 진입당 1회 (immediate_open·scroll_ack 중복 방지) */
  ...
  useEffect(() => {
    ...
    const applyOptimisticRoomRead = (snap: CommunityMessengerRoomSnapshot, lastReadMessageId: string | null) => {
      ...
      const remainingUnread = countRemainingUnreadAfterCursor({
        messages,
        viewerLastReadMessageId: lastReadMessageId,
      });
      ...
      if (lastReadMessageId) {
        useMessengerRoomReaderStateStore.getState().setLastVisibleMessageId(id, lastReadMessageId);
      }
    ...
    const flushRoomReadAck = (reason: RoomReadAckReason, lastReadMessageId: string) => {
      ...
      const previousCursor =
        roomOpenMarkReadRef.current.lastMarkedMessageId ?? snap.viewerLastReadMessageId ?? null;
      if (
        !isReadCursorMonotonicAdvance({
          messages,
          previousCursorId: previousCursor,
    ...
    const resolveReadCandidate = (reason: RoomReadAckReason): string | null => {
      ...
      const previousCursor =
        ...
        snap.viewerLastReadMessageId ??
        null;
      let visibleCandidate = lastVisibleMarkableMessageId(
        vp,
      ...
      if (
        visibleCandidate &&
        !isReadCursorMonotonicAdvance({
          messages: roomMessagesRef.current.length > 0 ? roomMessagesRef.current : snap.messages ?? [],
          previousCursorId: previousCursor,
      ...
      if (!state.readable || !viewportOk) {
        ...
        debugRoomReadAck({
          ...
          focused: state.focused,
          rendered: state.rendered,
          hasDom: Boolean(visibleCandidate),
          nearBottom,
          lastVisibleMessageId: visibleCandidate,
          previousReadCursor: previousCursor,
          nextReadCursor: visibleCandidate,
          debounceMs: CM_MARK_READ_SCROLL_DEBOUNCE_MS,
    ...
    /**
     * Proven root (clean baseline CASE A): unconditional clearScheduledReadAck() at every
     * schedule enter cancelled a pending debounce flush; tryEarlyOptimistic then remounted
     * the effect (unreadCount in readGateVersion) and cleanup cleared the timer → FLUSH 0.
     * Repair: coalesce schedules into one rAF resolve; on valid candidate, flush immediately
     * in the same turn (fetch starts before remount). Do not debounce past flush ownership.
     */
    let latestScheduleReason: RoomReadAckReason = "initial-render";
    const scheduleRoomReadAck = (reason: RoomReadAckReason) => {
      latestScheduleReason = reason;
      if (readAckRafId != null) {
        return;
      }
      if (readAckDebounceTimer != null) {
      ...
      const run = () => {
        readAckRafId = null;
        if (cancelled) return;
        const scheduleReason = latestScheduleReason;
        const candidate = resolveReadCandidate(scheduleReason);
        if (!candidate) {
          /**
        ...
        if (
          ...
          tryEarlyOptimisticListBadgeClear(scheduleReason, candidate, snapEarly);
        }
        flushRoomReadAck(scheduleReason, candidate);
      };
