...
import { RECOMMENDATION_EXPERIMENTS_SETTINGS_KEY } from "@/lib/recommendation-experiments/recommendation-experiments-keys";
...
function parseBundle(raw: unknown): RecommendationExperimentsBundleV1 | null {
  if (raw == null || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  if (o.version !== 1 || !Array.isArray(o.feedVersions) || !Array.isArray(o.experiments)) return null;
  return o as RecommendationExperimentsBundleV1;
}

export async function loadRecommendationExperimentsBundleFromDb(
  sb: SupabaseClient
): Promise<
  | { ok: true; bundle: RecommendationExperimentsBundleV1; source: "db" | "default" }
  | { ok: false; reason: "missing_table" | "error"; message?: string }
> {
  const { data, error } = await sb
    .from("admin_settings")
    .select("value_json")
    .eq("key", RECOMMENDATION_EXPERIMENTS_SETTINGS_KEY)
    .maybeSingle();
  ...
  if (!parsed) {
    return { ok: true, bundle: createDefaultRecommendationExperimentsBundle(), source: "default" };
  }
  return { ok: true, bundle: parsed, source: "db" };
}

export async function saveRecommendationExperimentsBundleToDb(
  sb: SupabaseClient,
  ...
  const { error } = await sb.from("admin_settings").upsert(
    {
      key: RECOMMENDATION_EXPERIMENTS_SETTINGS_KEY,
      value_json,
