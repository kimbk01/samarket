...
export const PRELAUNCH_RESET_SELECTIVE_MATRIX: readonly PrelaunchResetSelectiveMatrixRow[] = [
  ...
  },
  {
    ...
    dependencies: "soft tombstone; trade/order detach-only",
    financeRisk: "gate",
    storage: "none",
    auth: "none",
    protection: "trade / store_order never wiped; B4 soft for general_direct/group",
    executionSupport:
      "B4: UPDATE deleted_at soft tombstone for general_direct|group by explicit chatRoomIds — hard DELETE forbidden in Prelaunch default",
    reasonKo: "부분 — 일반/그룹 soft tombstone (거래·주문 detach · hard는 HIGH-RISK 전용)",
    reasonEn: "Partial — general/group soft tombstone (trade/order detach · hard = HIGH-RISK only)",
  },
  {
    key: "friend",
    group: "content",
    labelKo: "친구 / 차단 관계",
    labelEn: "Friend / block graph",
    support: "NOT_SUPPORTED",
    selectAllEligible: false,
    dbOwner: "user_social_relations",
    dependencies: "profiles endpoints only — never chat/rooms/finance",
    financeRisk: "none",
    storage: "none",
    auth: "none",
    protection: "B5 Reset SSOT · either_endpoint; chat/member/finance preserved",
    executionSupport:
      "Data Reset SSOT: lib/admin/data-reset (friend:user either_endpoint · friend:all). Prelaunch selective checkbox remains NOT_SUPPORTED — use /admin/system/data-reset.",
    reasonKo: "Data Reset SSOT로 이전 — Prelaunch 선택 실행 없음",
    reasonEn: "Moved to Data Reset SSOT — not Prelaunch selective execute",
  },
  {
    key: "orders",
    group: "commerce",
