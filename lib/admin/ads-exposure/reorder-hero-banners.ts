...
export type BannerSlideOrderItem = { campaignId: string; sortOrder: number };
...
export async function reorderDeliveryHeroBannerSlides(
  ...
  const occupants = await loadHeroOccupancyCampaigns(sb);
  const heroLive = occupants.filter((c) =>
    c.inventoryKeys.includes(DELIVERY_HERO_PLACEMENT_KEY)
  );
  const heroIdSet = new Set(heroLive.map((c) => c.id));

  // Allow reorder of any banner campaigns that are HERO-linked (incl. paused).
  ...
  // Soft warn if reordering campaigns not currently HERO-occupying — still allow (ops may order scheduled).
  void heroIdSet;
