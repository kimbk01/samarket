...
const MATRIX: Case[] = [
  {
    name: "A legacy super_admin, no membership",
...
describe("ADMIN AUTHORITY SKEW CUTOVER — CURRENT dual-read matrix", () => {
  it.each(MATRIX)("$name → allow=$expectAllow (skew=0 across readers)", async (c) => {
    ...
    // Same CURRENT formula used by requireAdmin / isRouteAdmin / platform-admin-db
    expect(allow).toBe(c.expectAllow);

    // Re-run without profileRole arg (DB profile fetch path) — still same result
    const effectiveDb = await resolveEffectiveAdminRole(sb, "u1");
    expect(isPrivilegedAdminRole(effectiveDb)).toBe(c.expectAllow);
  });

  it("source: isRouteAdmin / platform-admin-db / staff list share CURRENT helper", () => {
