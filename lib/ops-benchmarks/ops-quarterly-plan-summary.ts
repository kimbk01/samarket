...
import { getOpsQuarterlyPlans, getCurrentQuarter } from "./ops-benchmarks-state";
import type { OpsQuarterlyPlanSummary } from "@/lib/types/ops-benchmarks";

export function getOpsQuarterlyPlanSummary(year?: number): OpsQuarterlyPlanSummary {
  ...
  const latestUpdatedAt =
    list.length > 0
      ? list.reduce((max, p) => (p.updatedAt > max ? p.updatedAt : max), list[0].updatedAt)
      : null;
