...
import { SERVICE_CATEGORIES_SETTINGS_KEY } from "@/lib/admin-settings/service-categories-keys";
import type { ServiceCategoriesBundleV1 } from "@/lib/admin-settings/service-categories-state";
import { createDefaultServiceCategoriesBundle } from "@/lib/admin-settings/service-categories-state";
...
function parseBundle(raw: unknown): ServiceCategoriesBundleV1 | null {
  if (raw == null || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  if (o.version !== 1 || !Array.isArray(o.categories) || !Array.isArray(o.subcategories)) {
    return null;
  }
  return o as ServiceCategoriesBundleV1;
}

export async function loadServiceCategoriesBundleFromDb(
  sb: SupabaseClient
): Promise<
  | { ok: true; bundle: ServiceCategoriesBundleV1; source: "db" | "default" }
  | { ok: false; reason: "missing_table" | "error"; message?: string }
> {
  const { data, error } = await sb
    .from("admin_settings")
    .select("value_json")
    .eq("key", SERVICE_CATEGORIES_SETTINGS_KEY)
    .maybeSingle();
  ...
  if (!parsed) {
    return { ok: true, bundle: createDefaultServiceCategoriesBundle(), source: "default" };
  }
  return { ok: true, bundle: parsed, source: "db" };
}

export async function saveServiceCategoriesBundleToDb(
  sb: SupabaseClient,
  ...
  const { error } = await sb.from("admin_settings").upsert(
    {
      key: SERVICE_CATEGORIES_SETTINGS_KEY,
      value_json,
