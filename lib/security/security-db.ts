...
import { SECURITY_OPS_SETTINGS_KEY } from "@/lib/security/security-keys";
import type { SecurityOpsBundleV1 } from "@/lib/security/security-state";
import { createDefaultSecurityOpsBundle } from "@/lib/security/security-state";
...
function parseBundle(raw: unknown): SecurityOpsBundleV1 | null {
  if (raw == null || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  if (o.version !== 1 || !Array.isArray(o.checks) || !Array.isArray(o.issues)) {
    return null;
  }
  return o as SecurityOpsBundleV1;
}

export async function loadSecurityOpsBundleFromDb(
  sb: SupabaseClient
): Promise<
  | { ok: true; bundle: SecurityOpsBundleV1; source: "db" | "default" }
  | { ok: false; reason: "missing_table" | "error"; message?: string }
> {
  const { data, error } = await sb
    .from("admin_settings")
    .select("value_json")
    .eq("key", SECURITY_OPS_SETTINGS_KEY)
    .maybeSingle();
  ...
  if (!parsed) {
    return { ok: true, bundle: createDefaultSecurityOpsBundle(), source: "default" };
  }
  return { ok: true, bundle: parsed, source: "db" };
}

export async function saveSecurityOpsBundleToDb(
  sb: SupabaseClient,
