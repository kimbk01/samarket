...
import { PRODUCT_BACKLOG_SETTINGS_KEY } from "@/lib/product-backlog/product-backlog-keys";
...
function parseBundle(raw: unknown): ProductBacklogBundleV1 | null {
  ...
  return o as ProductBacklogBundleV1;
}

export async function loadProductBacklogBundleFromDb(
  sb: SupabaseClient
): Promise<
  | { ok: true; bundle: ProductBacklogBundleV1; source: "db" | "default" }
  | { ok: false; reason: "missing_table" | "error"; message?: string }
> {
  const { data, error } = await sb
    .from("admin_settings")
    .select("value_json")
    .eq("key", PRODUCT_BACKLOG_SETTINGS_KEY)
    .maybeSingle();
  ...
  if (!parsed) {
    return { ok: true, bundle: createDefaultProductBacklogBundle(), source: "default" };
  }
  return { ok: true, bundle: parsed, source: "db" };
}

export async function saveProductBacklogBundleToDb(
  sb: SupabaseClient,
  ...
  const { error } = await sb.from("admin_settings").upsert(
    {
      key: PRODUCT_BACKLOG_SETTINGS_KEY,
      value_json,
