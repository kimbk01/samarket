...
import { LAUNCH_READINESS_SETTINGS_KEY } from "@/lib/launch-readiness/launch-readiness-keys";
import type { LaunchReadinessBundleV1 } from "@/lib/launch-readiness/launch-readiness-state";
import { createDefaultLaunchReadinessBundle } from "@/lib/launch-readiness/launch-readiness-state";
...
function parseBundle(raw: unknown): LaunchReadinessBundleV1 | null {
  if (raw == null || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  if (
    o.version !== 1 ||
    !Array.isArray(o.checklistTemplates) ||
    !Array.isArray(o.readinessItems) ||
    !Array.isArray(o.approvals) ||
    !Array.isArray(o.blockerLogs)
  ) {
    return null;
  }
  return o as LaunchReadinessBundleV1;
}

export async function loadLaunchReadinessBundleFromDb(
  sb: SupabaseClient
): Promise<
  | { ok: true; bundle: LaunchReadinessBundleV1; source: "db" | "default" }
  | { ok: false; reason: "missing_table" | "error"; message?: string }
> {
  const { data, error } = await sb
    .from("admin_settings")
    .select("value_json")
    .eq("key", LAUNCH_READINESS_SETTINGS_KEY)
    .maybeSingle();
  ...
  if (!parsed) {
    return { ok: true, bundle: createDefaultLaunchReadinessBundle(), source: "default" };
  }
  return { ok: true, bundle: parsed, source: "db" };
}

export async function saveLaunchReadinessBundleToDb(
  sb: SupabaseClient,
  ...
  const { error } = await sb.from("admin_settings").upsert(
    {
      key: LAUNCH_READINESS_SETTINGS_KEY,
      value_json,
