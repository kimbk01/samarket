 ...
 */
export function useInlineWriteSheetNavigationGuard(): {
  /** 이동할 URL을 넘기면 확인 후 해당 경로로 `router.push`(거래·필라이프 시트) */
  ...
  const guardBeforeNavigate = useCallback(
    (nextHref?: string) => {
      if (philifeWrite && !philifeWrite.attemptLeaveForExternalNavigation(nextHref)) return false;
      if (tradeWrite && !tradeWrite.attemptLeaveForExternalNavigation(nextHref)) return false;
      return true;
    },
