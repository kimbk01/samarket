...
import { OPS_KNOWLEDGE_SETTINGS_KEY } from "@/lib/ops-knowledge/ops-knowledge-keys";
import type { OpsKnowledgeBundleV1 } from "@/lib/ops-knowledge/ops-knowledge-state";
import { createDefaultOpsKnowledgeBundle } from "@/lib/ops-knowledge/ops-knowledge-state";
...
function parseBundle(raw: unknown): OpsKnowledgeBundleV1 | null {
  if (raw == null || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  if (
    o.version !== 1 ||
    !Array.isArray(o.searchLogs) ||
    !Array.isArray(o.recommendationLogs) ||
    !Array.isArray(o.recentViews)
  ) {
    return null;
  }
  return o as OpsKnowledgeBundleV1;
}

export async function loadOpsKnowledgeBundleFromDb(
  sb: SupabaseClient
): Promise<
  | { ok: true; bundle: OpsKnowledgeBundleV1; source: "db" | "default" }
  | { ok: false; reason: "missing_table" | "error"; message?: string }
> {
  const { data, error } = await sb
    .from("admin_settings")
    .select("value_json")
    .eq("key", OPS_KNOWLEDGE_SETTINGS_KEY)
    .maybeSingle();
  ...
  if (!parsed) {
    return { ok: true, bundle: createDefaultOpsKnowledgeBundle(), source: "default" };
  }
  return { ok: true, bundle: parsed, source: "db" };
}

export async function saveOpsKnowledgeBundleToDb(
  sb: SupabaseClient,
  ...
  const { error } = await sb.from("admin_settings").upsert(
    {
      key: OPS_KNOWLEDGE_SETTINGS_KEY,
      value_json,
