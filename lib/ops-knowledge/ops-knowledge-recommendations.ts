/**
 * 운영 지식베이스 문서 추천 (linkedType/category/tag 기반)
 */
import type {
...
function scoreByMatch(
  ...
  if (item.category === "incident_response" && ["incident", "fallback", "kill_switch"].includes(sourceType))
    s += 0.2;
...
export function getOpsKnowledgeRecommendations(
  ...
  const items: OpsKnowledgeRecommendationItem[] = scored.slice(0, limit).map(
    ({ item, score, reasonLabel: r }) => ({
