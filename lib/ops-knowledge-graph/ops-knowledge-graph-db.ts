...
import { OPS_KNOWLEDGE_GRAPH_SETTINGS_KEY } from "@/lib/ops-knowledge-graph/ops-knowledge-graph-keys";
import type { OpsKnowledgeGraphBundleV1 } from "@/lib/ops-knowledge-graph/ops-knowledge-graph-state";
import { createDefaultOpsKnowledgeGraphBundle } from "@/lib/ops-knowledge-graph/ops-knowledge-graph-state";
...
function parseBundle(raw: unknown): OpsKnowledgeGraphBundleV1 | null {
  ...
  if (
    ...
    !Array.isArray(o.resolutionCases) ||
    !Array.isArray(o.documentRankings) ||
    !Array.isArray(o.similarRecommendations)
  ) {
    return null;
  }
  return o as OpsKnowledgeGraphBundleV1;
}

export async function loadOpsKnowledgeGraphBundleFromDb(
  sb: SupabaseClient
): Promise<
  | { ok: true; bundle: OpsKnowledgeGraphBundleV1; source: "db" | "default" }
  | { ok: false; reason: "missing_table" | "error"; message?: string }
> {
  const { data, error } = await sb
    .from("admin_settings")
    .select("value_json")
    .eq("key", OPS_KNOWLEDGE_GRAPH_SETTINGS_KEY)
    .maybeSingle();
  ...
  if (!parsed) {
    return { ok: true, bundle: createDefaultOpsKnowledgeGraphBundle(), source: "default" };
  }
  return { ok: true, bundle: parsed, source: "db" };
}

export async function saveOpsKnowledgeGraphBundleToDb(
  sb: SupabaseClient,
  ...
  const { error } = await sb.from("admin_settings").upsert(
    {
      key: OPS_KNOWLEDGE_GRAPH_SETTINGS_KEY,
      value_json,
