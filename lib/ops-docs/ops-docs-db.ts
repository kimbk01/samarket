...
import { OPS_DOCS_SETTINGS_KEY } from "@/lib/ops-docs/ops-docs-keys";
import type { OpsDocsBundleV1 } from "@/lib/ops-docs/ops-docs-state";
import { createDefaultOpsDocsBundle } from "@/lib/ops-docs/ops-docs-state";
...
function parseBundle(raw: unknown): OpsDocsBundleV1 | null {
  if (raw == null || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  if (
    o.version !== 1 ||
    !Array.isArray(o.documents) ||
    !Array.isArray(o.documentSteps) ||
    !Array.isArray(o.documentLogs)
  ) {
    return null;
  }
  return o as OpsDocsBundleV1;
}

export async function loadOpsDocsBundleFromDb(
  sb: SupabaseClient
): Promise<
  | { ok: true; bundle: OpsDocsBundleV1; source: "db" | "default" }
  | { ok: false; reason: "missing_table" | "error"; message?: string }
> {
  const { data, error } = await sb
    .from("admin_settings")
    .select("value_json")
    .eq("key", OPS_DOCS_SETTINGS_KEY)
    .maybeSingle();
  ...
  if (!parsed) {
    return { ok: true, bundle: createDefaultOpsDocsBundle(), source: "default" };
  }
  return { ok: true, bundle: parsed, source: "db" };
}

export async function saveOpsDocsBundleToDb(
  sb: SupabaseClient,
