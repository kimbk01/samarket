/**
 * CR1 chat rooms regression guards.
 */

export type ChatRoomsSnapshotBreakdown = {
  route: string;
  total_ms: number;
  db_ms: number;
  round_trips: number;
...
export type ChatRoomsRegressionAlert = {
  ...
  room_ordering_mismatch: 0 | 1;
  sequential_await_detected: 0 | 1;
  alerts: string[];
};

export function evaluateChatRoomsRegressionGuards(
  row: ChatRoomsSnapshotBreakdown
): ChatRoomsRegressionAlert {
  const alerts: string[] = [];
  ...
  if (row.repeated_join_detected && !row.fallback_used) {
    alerts.push("repeated_participant_profile_trade_join");
  }
  ...
  if (row.reconnect_path_used && row.fallback_used) alerts.push("reconnect_fallback_rooms");

  const alert: ChatRoomsRegressionAlert = {
  ...
  if (alerts.length > 0) {
    ...
    console.warn("[chat-rooms-regression-alert]", { ...alert, breakdown: row });
  }
  return alert;
}
