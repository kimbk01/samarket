...
export async function loadChatRoomDetailForUser(input: {
  ...
  if (!error && r && (r.seller_id === input.userId || r.buyer_id === input.userId)) {
    ...
    if (crSame) {
      ...
      if (crIds.includes(input.userId)) {
        ...
        const payload = {
          ...
          partnerUsername: partnerDisp2.partnerUsername ?? null,
          partnerDisplayName: partnerDisp2.partnerDisplayName ?? null,
    ...
    const payload = {
      ...
      partnerUsername: partnerDispPc.partnerUsername ?? null,
      partnerDisplayName: partnerDispPc.partnerDisplayName ?? null,
  ...
  if (roomType === "store_order") {
    ...
    if (oid) {
      ...
      const payload = {
        ...
        buyerId: bRow,
        sellerId: sRow,
        partnerUsername: partnerDispSo?.partnerUsername ?? null,
        partnerDisplayName: partnerDispSo?.partnerDisplayName ?? null,
  ...
  const payload = {
    ...
    buyerId: crRow.buyer_id ?? "",
    sellerId: crRow.seller_id ?? "",
    partnerUsername: partnerDispTrade.partnerUsername ?? null,
    partnerDisplayName: partnerDispTrade.partnerDisplayName ?? null,
    partnerNickname: partnerNickname2.trim() || (partnerId2 ?? "").slice(0, 8),
