...
import { OPS_MATURITY_SETTINGS_KEY } from "@/lib/ops-maturity/ops-maturity-keys";
import type { OpsMaturityBundleV1 } from "@/lib/ops-maturity/ops-maturity-state";
import { createDefaultOpsMaturityBundle } from "@/lib/ops-maturity/ops-maturity-state";
...
function parseBundle(raw: unknown): OpsMaturityBundleV1 | null {
  if (raw == null || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  if (
    o.version !== 1 ||
    !Array.isArray(o.maturityScores) ||
    !Array.isArray(o.teamKpis) ||
    !Array.isArray(o.roadmapItems)
  ) {
    return null;
  }
  return o as OpsMaturityBundleV1;
}

export async function loadOpsMaturityBundleFromDb(
  sb: SupabaseClient
): Promise<
  | { ok: true; bundle: OpsMaturityBundleV1; source: "db" | "default" }
  | { ok: false; reason: "missing_table" | "error"; message?: string }
> {
  const { data, error } = await sb
    .from("admin_settings")
    .select("value_json")
    .eq("key", OPS_MATURITY_SETTINGS_KEY)
    .maybeSingle();
  ...
  if (!parsed) {
    return { ok: true, bundle: createDefaultOpsMaturityBundle(), source: "default" };
  }
  return { ok: true, bundle: parsed, source: "db" };
}

export async function saveOpsMaturityBundleToDb(
  sb: SupabaseClient,
