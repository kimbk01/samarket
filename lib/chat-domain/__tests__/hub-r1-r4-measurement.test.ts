...
import {
  ...
  hubQuarantineIdsDeleted,
  hubQuarantineIdsMarkedRemoveNow,
} from "@/lib/chat-domain/projections/hub-r1-r4-measurement";

describe("Hub R1–R4 measurement", () => {
  it("Phase J1: R1+R4 deleted; R2–R3 keep; no remove_now remaining", () => {
    expect(hubQuarantineIdsMarkedRemoveNow()).toEqual([]);
    expect(hubQuarantineIdsDeleted()).toEqual(["R1", "R4"]);
