...
import { SYSTEM_OPS_SETTINGS_KEY } from "@/lib/system/system-ops-keys";
import type { SystemOpsBundleV1 } from "@/lib/system/system-state";
import { createDefaultSystemOpsBundle } from "@/lib/system/system-state";
...
function parseBundle(raw: unknown): SystemOpsBundleV1 | null {
  if (raw == null || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  if (
    o.version !== 1 ||
    !Array.isArray(o.systemHealth) ||
    o.operationStatus == null ||
    typeof o.operationStatus !== "object"
  ) {
    return null;
  }
  return o as SystemOpsBundleV1;
}

export async function loadSystemOpsBundleFromDb(
  sb: SupabaseClient
): Promise<
  | { ok: true; bundle: SystemOpsBundleV1; source: "db" | "default" }
  | { ok: false; reason: "missing_table" | "error"; message?: string }
> {
  const { data, error } = await sb
    .from("admin_settings")
    .select("value_json")
    .eq("key", SYSTEM_OPS_SETTINGS_KEY)
    .maybeSingle();
  ...
  if (!parsed) {
    return { ok: true, bundle: createDefaultSystemOpsBundle(), source: "default" };
  }
  return { ok: true, bundle: parsed, source: "db" };
}

export async function saveSystemOpsBundleToDb(
  sb: SupabaseClient,
