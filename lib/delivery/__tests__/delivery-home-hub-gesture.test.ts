...
import {
  ...
  shouldDeliveryHomeHubShortTapToggleDial,
} from "@/lib/delivery/delivery-home-hub-gesture";

describe("delivery-home-hub-gesture", () => {
  it("짧은 탭 pointerup 은 다이얼 토글", () => {
    expect(shouldDeliveryHomeHubShortTapToggleDial(false)).toBe(true);
  });

  it("롱프레스 직후 pointerup 은 토글하지 않음", () => {
    expect(shouldDeliveryHomeHubShortTapToggleDial(true)).toBe(false);
  });
