...
import {
  computePhilifePullPxFromTouchDy,
  getPhilifePullRefreshSnapshot,
  patchPhilifePullRefresh,
  PHILIFE_PULL_REFRESH_THRESHOLD_PX,
  runPhilifePullRefresh,
...
/**
 * CONTRACT — `/philife` PTR: scrollTop=0 이면 세로 당김 → 1단·2단(주제 탭) 사이 힌트 슬롯 확장.
 * 리스너: `[data-main-hub-scroll-body]` only. `pathname` 기준 단일 도메인과 함께 사용.
 */
export function usePhilifePullRefresh(enabled: boolean): void {
  ...
  useEffect(() => {
    ...
    const resetPull = () => {
      ...
      if (pullRef.current === 0 && getPhilifePullRefreshSnapshot().pullPx === 0) return;
      pullRef.current = 0;
      patchPhilifePullRefresh({ pullPx: 0 });
    };

    const onTouchStart = (e: TouchEvent) => {
      if (getPhilifePullRefreshSnapshot().refreshing) return;
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      const next = computePhilifePullPxFromTouchDy(dy);
      pullRef.current = next;
      if (next > PHILIFE_PULL_REFRESH_THRESHOLD_PX * 0.25) suppressClickRef.current = true;
      patchPhilifePullRefresh({ pullPx: next });
    };
    ...
    const onTouchEnd = () => {
      ...
      if (pulled >= PHILIFE_PULL_REFRESH_THRESHOLD_PX) {
        pullRef.current = 0;
        void runPhilifePullRefresh(pulled);
        return;
      }
      resetPull();
    };
    ...
    const retryId = window.setTimeout(() => {
      ...
      if (retryRoot instanceof HTMLElement) {
        retryRoot.dataset.philifePtrRoot = "1";
      }
    }, 0);
    ...
    return () => {
      ...
      if (cleanupRoot instanceof HTMLElement) {
        delete cleanupRoot.dataset.philifePtrRoot;
      }
      resetPull();
      if (getPhilifePullRefreshSnapshot().refreshing) {
