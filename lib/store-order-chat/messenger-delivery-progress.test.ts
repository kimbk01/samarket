...
describe("messengerDeliveryProgressFillRatio", () => {
  it("accepted fills toward 준비(조리)중", () => {
    ...
    expect(messengerDeliveryProgressFillRatio("accepted", "local_delivery")).toBeCloseTo(1 / 6, 5);
  });

  it("pending fills toward 주문접수", () => {
    expect(messengerDeliveryProgressFillRatio("pending", "local_delivery")).toBe(0);
  });
