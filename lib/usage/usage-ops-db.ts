...
import { USAGE_OPS_SETTINGS_KEY } from "@/lib/usage/usage-ops-keys";
import type { UsageOpsBundleV1 } from "@/lib/usage/usage-state";
import { createDefaultUsageOpsBundle } from "@/lib/usage/usage-state";
...
function parseBundle(raw: unknown): UsageOpsBundleV1 | null {
  if (raw == null || typeof raw !== "object") return null;
  const o = raw as Record<string, unknown>;
  if (o.version !== 1 || !Array.isArray(o.usageMetrics)) return null;
  return o as UsageOpsBundleV1;
}

export async function loadUsageOpsBundleFromDb(
  sb: SupabaseClient
): Promise<
  | { ok: true; bundle: UsageOpsBundleV1; source: "db" | "default" }
  | { ok: false; reason: "missing_table" | "error"; message?: string }
> {
  const { data, error } = await sb
    .from("admin_settings")
    .select("value_json")
    .eq("key", USAGE_OPS_SETTINGS_KEY)
    .maybeSingle();
  ...
  if (!parsed) {
    return { ok: true, bundle: createDefaultUsageOpsBundle(), source: "default" };
  }
  return { ok: true, bundle: parsed, source: "db" };
}

export async function saveUsageOpsBundleToDb(
  sb: SupabaseClient,
