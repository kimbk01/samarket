/**
 * 첫 주 관제 요약 (open/critical issue → stability status)
 */
...
export function getLaunchWeekSummary(): LaunchWeekSummary {
  ...
  if (criticalIssueCount > 0 || killSwitchToday > 0)
    currentStabilityStatus = "critical";
  else if (openIssueCount >= 3 || blockedChecklistCount > 0)
    currentStabilityStatus = "warning";
  else if (openIssueCount >= 1 || fallbackToday > 0)
    currentStabilityStatus = "watch";
