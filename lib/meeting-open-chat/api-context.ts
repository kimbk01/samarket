...
export type MeetingOpenChatRoomContext = {
  sb: SupabaseClient<any>;
  meetingId: string;
  roomId: string;
  userId: string;
  member: MeetingOpenChatMemberAccess;
};

export async function loadMeetingOpenChatRoomContext(
  meetingId: string,
  roomId: string,
  userId: string
): Promise<{ ok: true; ctx: MeetingOpenChatRoomContext } | { ok: false; response: NextResponse }> {
  ...
  const roomMeta = await getMeetingOpenChatRoomMeetingId(sb, rid);
  if (!roomMeta.ok) {
  ...
  const member = await getActiveMeetingOpenChatMember(sb, rid, userId);
  if (!member.ok) {
