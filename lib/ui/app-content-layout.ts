...
/** 메인 스크롤 영역 최대 폭 + 가운데 정렬 */
export const APP_MAIN_COLUMN_CLASS =
  "mx-auto w-full min-w-0 max-w-lg sm:max-w-xl md:max-w-2xl lg:max-w-3xl xl:max-w-4xl";
...
export const APP_TIER1_VIEWPORT_BLEED_FROM_COLUMN_CLASS =
  "relative min-w-0 w-screen max-w-[100vw] shrink-0 ml-[calc(50%-50vw)]";
...
/**
 * 거래 1단(RegionBar·카테고리 서브헤더 등) 내부 행 —
 * 본문 `APP_MAIN_COLUMN_CLASS`와 같은 max-w·거터로 좌우 끝을 맞춤(내용만 넓게 벌리지 않음).
 */
export const APP_MAIN_HEADER_INNER_CLASS = `${APP_MAIN_COLUMN_CLASS} ${APP_MAIN_GUTTER_X_CLASS}`;
