 ...
 * 렌터카 `rental-car` → `rent-car` (composition profile 과 WRITE/LIST skin 일치).
 */
export function resolveTradeWriteSkinKey(iconKey: string | null | undefined): string {
  ...
  if (k === "rental-car") return "rent-car";
...
export function isRentCarTradeWriteSkin(skinKey: string | null | undefined): boolean {
  const s = (skinKey ?? "").trim();
  return s === "rent-car" || s === "rental-car";
}
