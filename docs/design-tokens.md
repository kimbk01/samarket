...
| `--ui-danger` | 경고·삭제 |
| `--ui-success` | 성공 |
| `--ui-radius-rect` | **사각형** 카드·섹션·바텀시트 등 모서리 **4px** (아바타·FAB 등 `rounded-full` 은 제외) |
| `--ui-radius-pill` | 원형·pill 전용 (`rounded-full` 과 함께) |
...
- 사각형 모서리: **`rounded-ui-rect`** (= `4px`, 토큰 `--ui-radius-rect`)
- 시트 상단만: `rounded-t-[length:var(--ui-radius-rect)]` (또는 스크립트가 이미 적용한 동일 패턴)
- 원·pill: **`rounded-full`** 유지 (`--ui-radius-pill` 은 문서용; 실제는 `rounded-full`)

일괄 정렬에 `scripts/apply-ui-rect-radius.mjs` 사용(재실행 시 `rounded-full` 포함 줄은 건너뜀).
