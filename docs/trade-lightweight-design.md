...
**1순위 목표**는 “기능을 많이 넣기”가 아니라 **가볍게 유지**하는 것이다. **체감 속도**와 **유지 비용**을 우선하고, 기능은 필요한 만큼만, 데이터는 **한 번에·예측 가능하게** 내려준다.

**느려지기 쉬운 요인**(의도적으로 막는다):

- 오래 쌓인 코드·레이어 중복으로 **같은 데이터를 여러 경로**에서 가져오는 것
- **중복 요청**(마운트 직후 재 fetch, 효과 체인, 캐시 키 난립)
- **큰 번들**(페이지 전체 `use client`, 무거운 UI를 초기 청크에 포함)
