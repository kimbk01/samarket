...
SAMarket은 **상용 운영**을 전제로 한 빌더이므로, 거래 축의 **1순위 목표**는 “기능을 많이 넣기”가 아니라 **가볍게 유지**하는 것이다. **체감 속도**·**예측 가능한 데이터 경로**·**유지 비용**을 우선하고, 기능은 필요한 만큼만, 데이터는 **한 번에·일관되게** 내려준다.

**느려지기 쉬운 요인**(의도적으로 막는다 — 코드·리뷰에서 이 네 가지를 기본 점검한다):

- 오래 쌓인 코드·레이어 중복으로 **같은 데이터를 여러 경로**에서 가져오는 것 → **서버에서 한 번에**, 클라이언트는 **짧은 TTL 캐시·single-flight**로 중복 왕복 완화
- **중복 요청**(마운트 직후 재 fetch, 효과 체인, 캐시 키 난립)
- **큰 번들**(페이지 전체 `use client`, 무거운 UI를 초기 청크에 포함) → **페이지당 JS 줄이기**, 인터랙션만 섬·지연 로드
- **불필요한 클라이언트 상태·과한 쿼리** → URL·서버 응답 우선, **쿼리 단순화·캐싱**(HTTP·앱 메모리·DB 인덱스와 함께 검토)

**코드에 반영하는 네 가지 축**(이하 절·표와 1:1로 연결):

| 축 | 실천 방향 |
|----|-----------|
| **페이지당 JS 줄이기** | §2 — RSC·클라이언트 섬·지연 로드 |
| **서버에서 데이터 한 번에** | §3 — 라우트당 단일 로더·중복 fetch 금지 |
| **불필요한 클라이언트 상태 줄이기** | §4 — URL·`router.refresh()`·서버 응답 우선 |
| **쿼리 단순화·캐싱** | §5·§6 — 단순 쿼리·필드 슬림·HTTP·앱·DB 계층 |

상수·식별자(검색·리뷰용): [`lib/trade/trade-lightweight-goals.ts`](../lib/trade/trade-lightweight-goals.ts).
