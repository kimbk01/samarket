...
| 2026-07-28 | **실기기 keyboard authority 수정** — Timeline ref mount 뒤 ResizeObserver를 연결하고, bottom은 실제 geometry 변화당 1회 follow, 과거 위치는 visible message id+offset 보존. 동일 geometry 중복 scroll write 금지 | `messenger-room-scroll-anchor-controller`, `chat-thread-scroll/engine`, `chat-thread-scroll/__tests__/engine.test.ts` |
