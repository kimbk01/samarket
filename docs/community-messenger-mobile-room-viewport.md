...
| 2026-07-28 | **Unified entry+keyboard authority** — Timeline attach sync `flushInitialEntryAnchor`; stickRef↔engine sync; layout resize stick→force pin / !stick→visible-anchor; RO after mount; no keyboard-only scrollToBottom / no revert-cycle one-line patch | `engine.ts`, `messenger-room-scroll-anchor-controller`, phase1, MessageTimeline, `docs/chat-thread-scroll-contract.md` |
