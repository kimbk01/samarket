...
| 2026-07-27 | **iOS composer double offset fix** — shell=`visualViewport.height` 유지; keyboard open composer padding 항상 0 (iosOverlayKbOffset 재적용 제거); closed→`--safe-bottom` | `cm-room-visible-viewport-contract`, `use-cm-room-visible-viewport-shell` |
| 2026-07-27 | **iOS visualViewport band pin** — height-only at y=0 FAIL(composer 상단+대공백); shell `top=offsetTop`+`height=vv.height` (translateY 금지); focus `scrollIntoView` 제거; debug `__DIBAY_CM_ROOM_KB_DEBUG__` | `cm-room-visible-viewport-contract`, `use-cm-room-visible-viewport-shell`, `CommunityMessengerRoomPhase2Composer` |
