...
| 2026-06-20 | **Entry scroll Layout Settle Gate** — `entryInitialScrollDone`·terminal tail settle 후만 `entryScrollSettled`; prepend 중 keep-bottom 차단; 그룹 채팅 entry scroll; `verify:cm-room-entry-scroll-contract` | `messenger-room-entry-scroll-settle`, `messenger-room-entry-scroll-owner`, `messenger-room-scroll-anchor-controller`, `GroupChatRoomClient`, `docs/community-messenger-room-entry-scroll-contract.md` |
