...
`NODE_ENV === "development"` 일 때는 **운영과 동일한 백그라운드 선행 로드를 강제하지 않는다.**

- `router.prefetch` 다발 → dev 서버 **온디맨드 컴파일**과 겹침 → `shouldRunBottomNavProgrammaticPrefetch()` 가 false 이면 생략.
- 하단 탭 `Link prefetch` → 뷰포트 진입 시에도 컴파일이 자주 붙음 → `shouldEnableNextLinkPrefetchOnMainNav()` 로 dev 에서만 끔.
- Philife 글로벌 피드 워밍·홈 셸 예열 → `PhilifeFeedWarmPrefetch`, `warmMainShellData` 에서 dev 생략.
- 메신저 방 `CommunityMessengerRoomClient` 의 목록 `router.prefetch` → `shouldRunMessengerListRoutePrefetch()` 로 dev 생략.
