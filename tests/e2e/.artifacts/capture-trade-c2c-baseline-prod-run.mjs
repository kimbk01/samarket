 ...
 *
 * 전제: `npm run dev` (또는 PLAYWRIGHT 가 webServer 기동)
...
const outFile = path.join(outDir, "trade-c2c-baseline-prod-like.json");
...
async function waitForMessengerRoomAfterChatClick(page, chatClickAt) {
  ...
  try {
    ...
    failureReason = "room_url_not_reached";
  } finally {
...
async function main() {
  ...
  await context.addInitScript(() => {
    try {
      ...
      for (const k of ["location", "microphone", "speaker"]) {
  ...
  if (!chatRoomCapture.chat_room_url_reached && chatRoomCapture.failure_reason === "chat_not_attempted") {
    chatRoomCapture.failure_reason = hasPost ? "room_url_not_reached" : "no_post_link";
  }
  ...
  if (await firstRowTap.isVisible({ timeout: 8_000 }).catch(() => false)) {
    ...
    if (await ta.isVisible({ timeout: 15_000 }).catch(() => false)) {
      ...
      if (await sendBtn.isVisible().catch(() => false)) await sendBtn.click({ timeout: 15_000 }).catch(() => undefined);
  ...
  const metrics = {
    ...
    scenarios: {
      ...
      env: "prod-like (next build + next start), qqqq@manual.local, mobile 390px",
    },
  };
