-- =============================================================================
-- 디바이(DiBaY) 알림 정책 확장 — 수동 적용용 (붙여넣기)
--
-- 사용 방법:
--   1) Supabase Dashboard → SQL Editor 열기
--   2) 이 파일 전체 선택 → 복사 → 에디터에 붙여넣기
--   3) Run 실행
--
-- 참고:
--   - 멱등 설계입니다 (IF NOT EXISTS / DO 블록). 여러 번 실행해도 안전합니다.
--   - 동일 내용이 supabase/migrations/20260701100000_dibay_notification_policy_extend.sql
--     에 있으므로, CLI 마이그레이션으로 이미 적용했다면 생략해도 됩니다.
-- =============================================================================
...
-- 3) notifications 인박스 확장 (테이블이 있을 때만)
DO $$
BEGIN
...
-- 4) 관리자 알림 캠페인 (service_role·서버 API 전용 — RLS만 켜고 사용자 정책 없음)
