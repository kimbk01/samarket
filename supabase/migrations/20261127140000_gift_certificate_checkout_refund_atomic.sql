-- G7/G10: Gift checkout fold into create_store_order_atomic + refund atomic RPC
-- Financial Integrity Root Fix — removes post-order gift redeem.

ALTER TABLE public.store_orders
  ADD COLUMN IF NOT EXISTS amount_before_gift integer NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.store_orders.amount_before_gift IS
  'Payment due after coupon, before gift redemption (durable snapshot).';
...
COMMENT ON FUNCTION public.create_store_order_atomic(uuid, uuid, text, jsonb, jsonb) IS
  'Atomic order + coupon + gift certificate redeem (G7). gift_instance_ids in p_order; amounts server-side.';
...
BEGIN
  ...
  UPDATE public.store_orders
     SET order_status = 'refunded',
         ...
         updated_at = now()
   WHERE id = p_order_id
     AND order_status IS DISTINCT FROM 'refunded';
  IF NOT FOUND THEN
