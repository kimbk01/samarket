-- P0: active participants only for CM group send unread/recipients; return room_type.
...
begin
  ...
  inner join public.community_messenger_participants p
    ...
    and p.left_at is null and p.blocked_hidden_at is null
  ...
  if v_trim_client is not null then
    ...
    if v_existing_id is not null then
      ...
      where room_id = p_room_id and user_id <> p_sender_id
        and left_at is null and blocked_hidden_at is null;
      ...
      return jsonb_build_object(
        ...
        'room_type', to_jsonb(v_room.room_type)
      );
    end if;
  end if;
