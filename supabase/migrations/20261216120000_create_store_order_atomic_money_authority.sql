-- CUT-2 ROOT A — create_store_order_atomic money authority
-- Recompute items subtotal + delivery fee inside TX; assert payload; write authoritative amounts.
-- Preserves: client_order_key idempotency, stock/price/options CAS, coupon/gift semantics.
-- Does NOT change delivery fee policy, state machine, or Finance internals.

-- CUT-2 ROOT A — Delivery fee canonical calculator (mirrors resolveChargedDeliveryFeePhp)
-- DO NOT invent new fee policy; keep existing business_hours_json semantics.

CREATE OR REPLACE FUNCTION public.store_charged_delivery_fee_php(
  p_business_hours_json jsonb,
...
DECLARE
  ...
  v_fee numeric;
  v_free_over numeric;
  v_fulfillment text := coalesce(nullif(btrim(p_fulfillment), ''), 'pickup');
  v_o jsonb := coalesce(p_business_hours_json, '{}'::jsonb);
BEGIN
  IF v_fulfillment IS DISTINCT FROM 'local_delivery' THEN
    RETURN 0;
  END IF;

  v_mode := lower(nullif(btrim(coalesce(v_o->>'delivery_fee_mode', v_o->>'deliveryFeeMode', '')), ''));
  BEGIN
    v_fee := round(coalesce((v_o->>'delivery_fee_php')::numeric, (v_o->>'deliveryFeePhp')::numeric));
  EXCEPTION WHEN others THEN
    v_fee := NULL;
  END;
  BEGIN
    v_free_over := (v_o->>'free_delivery_over_php')::numeric;
  EXCEPTION WHEN others THEN
    BEGIN
      v_free_over := (v_o->>'freeDeliveryOverPhp')::numeric;
    EXCEPTION WHEN others THEN
      v_free_over := NULL;
    END;
  END;

  -- Infer mode when absent (same as parseCommerceExtrasFromHoursJson)
  IF v_mode IS NULL OR v_mode = '' THEN
    IF v_fee IS NOT NULL AND v_fee > 0 THEN
      v_mode := 'self';
    ELSIF nullif(btrim(coalesce(v_o->>'delivery_courier_label', v_o->>'deliveryCourierLabel', '')), '') IS NOT NULL THEN
      v_mode := 'courier';
    ELSIF v_fee = 0 THEN
      v_mode := 'self';
    ELSE
  ...
  IF v_mode = 'courier' OR v_mode = 'self_free_promo' THEN
    RETURN 0;
  END IF;
  ...
  IF v_fee < 0 THEN
    v_fee := 0;
  END IF;
  IF v_free_over IS NOT NULL AND v_free_over > 0 AND coalesce(p_items_subtotal, 0) >= v_free_over THEN
    RETURN 0;
  END IF;
  RETURN v_fee;
END;
...
REVOKE ALL ON FUNCTION public.store_charged_delivery_fee_php(jsonb, numeric, text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.store_charged_delivery_fee_php(jsonb, numeric, text) TO service_role;
