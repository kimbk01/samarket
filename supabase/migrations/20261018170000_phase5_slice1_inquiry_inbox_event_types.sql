-- Phase 5 Slice 1 (taxonomy A): inquiry_answered / inbox_message_received
-- Notes writer leaves admin_notice (Campaign keeps admin_notice).
-- Badge COUNT folds the new categories into admin_notice digit bucket.
...
COMMENT ON CONSTRAINT notification_events_type_check ON public.notification_events IS
  'Canonical notification registry event types; Phase 5 Slice 1 adds inquiry_answered + inbox_message_received.';
...
COMMENT ON FUNCTION public.count_notification_events_badge(uuid) IS
  '알림 뱃지 집계 — modern taxonomy + Phase 5 Slice 1 Inquiry/Inbox folded into admin_notice.';
