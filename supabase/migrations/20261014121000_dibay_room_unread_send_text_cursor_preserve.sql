-- Room Unread v1: preserve recipient last_read_at on text send (no cursor wipe)
-- Patches community_messenger_send_text_message from 20260912140000
...
COMMENT ON FUNCTION public.community_messenger_send_text_message(uuid, uuid, text, text, timestamptz, uuid) IS
  'CM text send — Room Unread v1: recipient last_read_at preserved (no wipe).';
