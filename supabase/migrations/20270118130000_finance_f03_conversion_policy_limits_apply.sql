-- F-03: ensure Coin→Cash conversion limit columns exist on Production.
-- Root cause: MIGRATION_NOT_APPLIED — repo had 20261208120000 but live lacked columns.
-- Idempotent ADD COLUMN IF NOT EXISTS + convert RPC with limits + post-settle balance (F-02).
...
-- CHECKs may already exist from 20261208120000; add only if missing (best-effort).
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'business_cash_conversion_rate_policies_minimum_coin_per_conversion_check'
  ) THEN
    ALTER TABLE public.business_cash_conversion_rate_policies
      ADD CONSTRAINT business_cash_conversion_rate_policies_minimum_coin_per_conversion_check
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'business_cash_conversion_rate_policies_conversion_unit_check'
  ) THEN
    ALTER TABLE public.business_cash_conversion_rate_policies
      ADD CONSTRAINT business_cash_conversion_rate_policies_conversion_unit_check
...
COMMENT ON COLUMN public.business_cash_conversion_rate_policies.minimum_coin_per_conversion IS
  'F-03 SSOT: Minimum Coin units required per conversion request.';
COMMENT ON COLUMN public.business_cash_conversion_rate_policies.conversion_unit IS
  'F-03 SSOT: Coin amount must be a multiple of this unit.';

-- Convert RPC: limits from policy SSOT + F-02 post-settle balance
...
-- Expose limits on rate RPC for Owner/Admin readers (same policy SSOT)
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'conversion_enabled', COALESCE(v_row.conversion_enabled, true),
    'minimum_coin_per_conversion', COALESCE(v_row.minimum_coin_per_conversion, 1),
    'conversion_unit', COALESCE(v_row.conversion_unit, 1),
    'maximum_conversions_per_day', v_row.maximum_conversions_per_day,
    'maximum_conversions_per_week', v_row.maximum_conversions_per_week,
    'maximum_conversions_per_month', v_row.maximum_conversions_per_month,
    'minimum_interval_hours', v_row.minimum_interval_hours,
    'daily_limit_coin', v_row.daily_limit_coin,
    'monthly_limit_coin', v_row.monthly_limit_coin
  );
END;
$$;

REVOKE ALL ON FUNCTION public.get_business_cash_conversion_rate() FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_business_cash_conversion_rate() FROM anon, authenticated;
