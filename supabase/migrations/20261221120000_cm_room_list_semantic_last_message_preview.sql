-- WHY_REQUIRED: dibay_append_room_message_atomic stored rooms.last_message = left(content),
-- so image/file tips persisted raw storage URLs. Room list preview SSOT must be semantic by
-- message_type + metadata (not URL-extension heuristics). Minimal function body change only.
--
-- Room list semantic tip: image → 사진; file → fileName or 파일; text → content (URLs stay text).
...
COMMENT ON FUNCTION public.dibay_append_room_message_atomic(
  ...
  'Room Unread v1.1h + semantic list tip: image/file last_message by type+metadata; text keeps content.';
