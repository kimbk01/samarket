-- Phase 5 — atomic store order create (stock + order + items + options + order_created event)
-- Single transaction; no app-layer compensate delete/restore on partial failure.
--
-- CONTRACT:
-- - Idempotent on (buyer_user_id, client_order_key): no double stock / items / event
-- - Tracked inventory: stock_qty >= qty CAS inside TX (concurrent → one PASS one FAIL)
-- - Non-tracked: no stock mutation
-- - Business FAIL paths that RETURN ok:false must not have mutated stock/order
--   (validate+lock first, then mutate). unique_violation after mutate rolls back via EXCEPTION.
...
COMMENT ON FUNCTION public.create_store_order_atomic(uuid, uuid, text, jsonb, jsonb) IS
  'Phase 5: atomic store order create — stock CAS + order + items + options + order_created event in one TX.';
