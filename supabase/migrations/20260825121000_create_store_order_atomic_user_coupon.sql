-- FREE COUPON CUT 7 — user_coupon redeem + funding snapshot in same TX.
-- CONTRACT:
...
BEGIN
  ...
  IF v_coupon_id IS NOT NULL THEN
    ...
    IF v_user_coupon_id IS NOT NULL THEN
      ...
      END IF;
    ELSE
      IF v_campaign.is_active IS NOT TRUE OR v_campaign.lifecycle_state IN ('paused', 'ended', 'revoked') THEN
    ...
    IF v_user_coupon_id IS NULL AND EXISTS (
      SELECT 1
