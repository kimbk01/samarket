-- CUT 6 — Wave ORDER BY / out_of_range / distance_km parity with OLD comparator.
-- Within-Gi sort must apply outOfRange before distance (G1 delivery-disabled included).
-- distance_km only when coverage.distance_applies (parity with evaluateDeliveryServiceability).
-- NO ranking cutover. Additive REPLACE of wave RPC only.
...
COMMENT ON FUNCTION public.get_store_discovery_shadow_wave(
  ...
  'CUT 6 bounded Gi×Dj wave — outOfRange-before-distance + distance_km only when distance_applies. No cutover.';
