-- Gift offer peer bind: replace min(uuid) with uuid-safe peer select.
-- FIRST_DIVERGENCE: Production gift_certificate_offer failed with
--   function min(uuid) does not exist
-- Scope: gift_certificate_offer ONLY. No money/redemption writer changes.
...
BEGIN
  ...
  -- PG: min(uuid) is unavailable on linked Postgres — pick peer without uuid aggregate.
