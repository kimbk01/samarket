-- CM bootstrap lite bundle: participant user_id 에 대한 first-paint profile labels (avatar·display) 1RTT embed.
-- bio·관계 메타는 제외 — payload_kb 35 이하 유지 목표.
...
comment on function public.community_messenger_bootstrap_lite_my_rooms_bundle(uuid, integer) is
  'CM bootstrap lite: rooms+participants+first-paint profile_labels(JSON) 단일 반환.';
