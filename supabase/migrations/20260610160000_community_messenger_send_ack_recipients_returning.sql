-- MP-AUDIT-13: send ACK — insert 경로 participants 선조회 제거, unread UPDATE RETURNING 으로 recipient_user_ids 집계.
...
BEGIN
  ...
  SELECT coalesce(
    to_jsonb(coalesce(array_agg(user_id::text ORDER BY user_id) FILTER (WHERE user_id <> p_sender_id), array[]::text[])),
    '[]'::jsonb
  )
    INTO v_recipients
  FROM unread_up;
...
COMMENT ON FUNCTION public.community_messenger_send_text_message(uuid, uuid, text, text, timestamptz, uuid) IS
  'CM 텍스트 전송: MP-AUDIT-8/13 ACK hot path — unread UPDATE RETURNING 으로 recipient 집계.';
