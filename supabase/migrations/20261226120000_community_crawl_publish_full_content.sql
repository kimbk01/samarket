-- V2-1: FULL_CONTENT operational publish contract
-- Expands publish_mode CHECK; adds dedicated RPC (legacy reference-summary untouched).
-- Does NOT write published post media rows (V2-5).

ALTER TABLE public.community_crawl_sources
  DROP CONSTRAINT IF EXISTS community_crawl_sources_publish_mode_check;

ALTER TABLE public.community_crawl_sources
  ADD CONSTRAINT community_crawl_sources_publish_mode_check
  CHECK (publish_mode IN ('REFERENCE_SUMMARY', 'FULL_CONTENT'));

COMMENT ON COLUMN public.community_crawl_sources.publish_mode IS
  'V2: FULL_CONTENT = operational Admin→DIBAY publish. REFERENCE_SUMMARY = legacy only.';
...
BEGIN
  ...
  END IF;
  -- FULL_CONTENT operational path: do NOT require source.publish_mode already flipped.
  -- Do NOT reject content that equals source body.
...
COMMENT ON FUNCTION public.community_crawl_publish_full_content(jsonb) IS
  'V2-1 operational crawler publish: atomic community_posts + community_crawl_post_links. Full dibay_body allowed (incl. identical to source). No post images. No point reward.';
