-- get_cm_bootstrap_full_snapshot social graph — user_social_relations, friend_requests empty
    ...
    WITH social AS (
      SELECT jsonb_build_object(
        'accepted_friends', coalesce((
          SELECT jsonb_agg(
            jsonb_build_object(
              'requester_id', sr.owner_user_id,
              'addressee_id', sr.target_user_id,
              'status', 'accepted',
              'created_at', sr.created_at,
              'responded_at', sr.created_at
            )
          )
          FROM public.user_social_relations sr
          WHERE sr.owner_user_id = p_user_id
            AND sr.relation_type = 'friend'
        ), '[]'::jsonb),
...
COMMENT ON FUNCTION public.get_cm_bootstrap_full_snapshot(uuid, text, integer, text) IS
  'FBT1 CM bootstrap — user_social_relations friend/blocked; friend_requests deprecated empty.';
