-- DIBAY Gift Certificate — STORE | PLATFORM gift_scope (B1)
-- Explicit scope enum; PLATFORM products/instances have null store_id.
-- Creation provenance nullable for legacy rows (do not fabricate).
...
ALTER TABLE public.gift_certificate_products
  ADD COLUMN IF NOT EXISTS gift_scope text,
  ADD COLUMN IF NOT EXISTS creation_source text,
  ADD COLUMN IF NOT EXISTS archived_at timestamptz NULL;

UPDATE public.gift_certificate_products
   SET gift_scope = 'STORE'
 WHERE gift_scope IS NULL;

ALTER TABLE public.gift_certificate_products
  ALTER COLUMN gift_scope SET DEFAULT 'STORE',
  ALTER COLUMN gift_scope SET NOT NULL;

ALTER TABLE public.gift_certificate_products
  DROP CONSTRAINT IF EXISTS gift_certificate_products_gift_scope_chk,
  ADD CONSTRAINT gift_certificate_products_gift_scope_chk
    CHECK (gift_scope IN ('STORE', 'PLATFORM'));

ALTER TABLE public.gift_certificate_products
  DROP CONSTRAINT IF EXISTS gift_certificate_products_creation_source_chk,
  ADD CONSTRAINT gift_certificate_products_creation_source_chk
    CHECK (
      creation_source IS NULL
      OR creation_source IN (
        'OWNER_APPLICATION',
        'ADMIN_DIRECT_STORE',
...
-- Relax store_id nullability for PLATFORM
ALTER TABLE public.gift_certificate_products
  ALTER COLUMN store_id DROP NOT NULL;

ALTER TABLE public.gift_certificate_products
  DROP CONSTRAINT IF EXISTS gift_certificate_products_scope_store_chk,
  ADD CONSTRAINT gift_certificate_products_scope_store_chk
    CHECK (
      (gift_scope = 'STORE' AND store_id IS NOT NULL)
      OR (gift_scope = 'PLATFORM' AND store_id IS NULL)
    );

CREATE INDEX IF NOT EXISTS gift_certificate_products_scope_idx
  ON public.gift_certificate_products (gift_scope, active, created_at DESC);

COMMENT ON COLUMN public.gift_certificate_products.gift_scope IS
  'STORE = single-store redeem; PLATFORM = eligible DIBAY stores. Never infer from null store_id alone.';
COMMENT ON COLUMN public.gift_certificate_products.creation_source IS
  'PRODUCT issuance provenance. Nullable for legacy rows — do not fabricate.';
COMMENT ON COLUMN public.gift_certificate_products.archived_at IS
  'Soft archive timestamp. Distinct from pause (active=false).';
...
ALTER TABLE public.gift_certificate_instances
  DROP CONSTRAINT IF EXISTS gift_certificate_instances_gift_scope_chk,
  ADD CONSTRAINT gift_certificate_instances_gift_scope_chk
    CHECK (gift_scope IN ('STORE', 'PLATFORM'));
...
ALTER TABLE public.gift_certificate_instances
  DROP CONSTRAINT IF EXISTS gift_certificate_instances_scope_store_chk,
  ADD CONSTRAINT gift_certificate_instances_scope_store_chk
    CHECK (
      (gift_scope = 'STORE' AND store_id IS NOT NULL)
      OR (gift_scope = 'PLATFORM' AND store_id IS NULL)
    );

CREATE INDEX IF NOT EXISTS gift_certificate_instances_scope_idx
  ON public.gift_certificate_instances (gift_scope, status, created_at DESC);

COMMENT ON COLUMN public.gift_certificate_instances.gift_scope IS
  'Denormalized from product at purchase. Redeem authority uses gift_scope + store_id contract.';

-- ---------------------------------------------------------------------------
-- Checkout eligibility helper (STORE same-store; PLATFORM any checkout store)
-- Store open/approved gates remain in create_store_order_atomic.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.gift_certificate_instance_allows_checkout_store(
  p_gift_scope text,
  p_instance_store_id uuid,
  p_checkout_store_id uuid
)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN p_checkout_store_id IS NULL THEN false
    WHEN coalesce(p_gift_scope, 'STORE') = 'STORE' THEN
      p_instance_store_id IS NOT NULL AND p_instance_store_id IS NOT DISTINCT FROM p_checkout_store_id
    WHEN coalesce(p_gift_scope, 'STORE') = 'PLATFORM' THEN
      true
    ELSE false
  END;
$$;

COMMENT ON FUNCTION public.gift_certificate_instance_allows_checkout_store(text, uuid, uuid) IS
  'STORE requires instance.store_id = checkout store. PLATFORM allows any checkout store (order gates apply separately).';

REVOKE ALL ON FUNCTION public.gift_certificate_instance_allows_checkout_store(text, uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.gift_certificate_instance_allows_checkout_store(text, uuid, uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.gift_certificate_instance_allows_checkout_store(text, uuid, uuid) TO authenticated;

-- ---------------------------------------------------------------------------
-- Purchase: copy gift_scope; PLATFORM store_id null
-- ---------------------------------------------------------------------------
