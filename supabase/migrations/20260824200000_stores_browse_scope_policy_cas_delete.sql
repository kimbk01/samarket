-- Browse scope CAS: support deleting secondary inherit stubs (canonical = no row).
...
CREATE OR REPLACE FUNCTION public.save_store_browse_scope_policy_cas(
  ...
  p_rows jsonb,
  p_actor_id uuid,
  p_delete_scope_keys text[] DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_delete_scope_keys IS NOT NULL AND cardinality(p_delete_scope_keys) > 0 THEN
    DELETE FROM public.store_browse_scope_policy
    WHERE scope_key = ANY (p_delete_scope_keys)
      AND sub_slug IS NOT NULL;
  END IF;
