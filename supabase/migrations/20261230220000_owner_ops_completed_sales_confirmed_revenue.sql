-- Owner completed-sales KPI: use confirmed merchant revenue (payment + gift + platform).
-- Mirrors lib/stores/confirmed-sale-revenue.ts for completed rows
-- (refunded/cancelled already excluded by order_status = 'completed'; store_orders has no refund_amount column).
-- DO NOT sum payment_amount alone — that is remaining customer payment after gift.
...
AS $$
  ...
  orders AS (
    SELECT
      ...
      o.dispute_status,
      o.payment_amount,
      coalesce(o.gift_redemption_amount, 0) AS gift_redemption_amount,
      coalesce(o.platform_funded_amount, 0) AS platform_funded_amount,
      o.created_at,
      o.updated_at,
  ...
  order_agg AS (
    SELECT
      ...
      coalesce(
        sum(
          CASE
            WHEN order_status = 'completed' AND updated_at >= (SELECT today_start FROM day_bounds) THEN
              greatest(
                0,
                coalesce(payment_amount, 0)
                  + coalesce(gift_redemption_amount, 0)
                  + coalesce(platform_funded_amount, 0)
              )
            ELSE 0
          END
        ),
...
COMMENT ON FUNCTION public.get_owner_store_ops_snapshot_counts(uuid) IS
  'Owner store order-counts snapshot — completed sales = confirmed merchant revenue (payment+gift+platform); store_ops meta remains separate TS query.';
